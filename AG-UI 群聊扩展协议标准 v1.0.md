...
消息可携带附件（图片 / 文档 / 二进制）。`text` 与 `document` 类附件（txt、md、源码等纯文本，以及 docx / xlsx / pptx / pdf 办公文档）由服务端提取全文注入智能体上下文（超长截断）；`image` 类（png / jpg / jpeg / gif / webp / bmp）在启用**图片理解（视觉）**时由服务端按字节 + MIME 读取、以 base64 多模态内容内联喂给视觉模型看图作答（未启用 / `mock` 提供方则仅携带元数据按文本处理）；其余 `binary` 类携带元数据供模型感知。
...
|发现客服知聚|`GET /ag-ui/group/discover`|无需其它参数（需身份）：返回全部客服知聚（对所有用户可见、可进入），每项含 isMember（是否已是成员）与 hasEntered（是否已登记为顾客参与者）|
|进入客服知聚|`POST /ag-ui/group/{groupId}/enter`|无 body；客服成员直接进入；非成员用户登记为<b>顾客参与者</b>（不加入成员表），随后即可订阅 / 读历史 / 发消息（独立会话）；普通知聚不可自行进入（403）|
...
GET **读接口鉴权**（快照 / 成员 / 历史分页，§5.4）要求登录，且校验**调用者能访问该群**——普通知聚为「该群成员」（未登录 401、非成员 403，群不存在 404）；**客服知聚**额外允许「已登记的顾客参与者」访问（见 §2.1.1，非成员见自己的会话）；`GET /ag-ui/member/{memberId}/groups` 仅本人可查（越权 403）。
...
**词面兜底与量纲（1.0.150+）**：BM25 的 sigmoid 分**不能直接当相似度用** —— 它零词面重叠就给 0.5，
于是“只共用一个常用词”也看着像 0.54，盖过一切阈值（实测：提问“公司食堂今天中午吃什么”、描述里恰好有“公司” → 0.537）。
现在词面命中会先换算到同一量纲（`Bm25Ranker.ToSimilarity`，零重叠→0）再过一条**固定底线 0.35**（不随库的严格度变）：
罕罕见词 / 专有号（实测换算后 0.42）仍能兜住，只共用一个常用词（0.07）不再算命中。
...
**试检索**：`POST /ag-ui/kb/{kbId}/search`（`{query, topK?, minScore?}`）供界面“调档位时立刻看影响”，
只回**片段预览**（截断 200 字），门槛可临时指定（先试再存）；权限与“能不能读这个库”一致。
**词面兜底与量纲**：同图库（见上）—— 换算到同一量纲后过固定底线 0.35，不随档位变。
