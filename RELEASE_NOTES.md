...
- **新增智能体调用链可视化**：含技能的智能体回复下方会显示一张可折叠的“智能体调用链”卡片，逐层展示<b>万事通 →(skill_hr) → hr专员 →(skill_handbook) → 员工手册解读专家</b>的完整调用链路，每层显示触发技能名、目标数字员工与传入请求，点开可看各层答复；链路随消息持久化（重启 / 刷新可回放）。**委派链一并可视化**：代为响应（向下委派/向上回退）的路径也以“代为响应”标记呈现在同一张调用链卡片里（总前台 → 一线-人力分线 → 人力专员，回退环显示“判定不符，向上回退”与宿主兑底），与技能调用区分展示。
- **新增“代为响应”条件委派**：可在数字员工里独立设置【语境不符时代为响应】与【代为响应数字员工】。被直接 @ 时，若该数字员工先判定“这不是我该回复的语境”，会委派给所选的数字员工代为回答（以本数字员工身份发出，标注“由 X 代为响应”）。**组织架构式委派 + 向上回退**：委派可多层（A→B→C…）；任一层若判定“语境不属于自己”且**无可向下委派对象（或下游整条链都判定不符）**，会**继续向上层回退**，直到最上层（被 @ 的宿主）兑底作答——不会没人回答，也不会硬答不符。带深度上限（≤4 层）与环路保护（A→B→A 不环回）。区别于整轮交接：只在该层判定语境不符时才委派 / 回退。
  **演示包**：`tools/agents-standin-demo.json` 内置“总前台 → 一线-人力分线 → 人力专员”等测试数字员工，导入后 @总前台：人力问题→向下多层委派；非人力问题→逐层向上回退、总前台兑底。可直接验证**向下委派与向上回退**。
...
- **New agent call-chain visualization**: under a skill-enabled agent's reply, a collapsible “agent call chain” card now shows the full nested invocation — 万事通 →(skill_hr) → hr专员 →(skill_handbook) → 员工手册解读专家 — with each level's skill name, target agent and the request sent; expand to view each level's reply. The chain is persisted with the message, so it can be replayed after refresh. **Delegation chains are visualized too**: the stand-in reply path (down-chain delegation / up-chain escalation) also appears in the same call-chain card, marked with a “Stand-in” tag (e.g. 总前台 → 一线-人力分线 → 人力专员; escalation hops show “judged out of scope, escalated up” and the host fallback), distinct from skill calls.
- **New “stand-in reply” conditional delegation**: a digital employee can set 【Reply on my behalf when out of scope】 plus a 【stand-in digital employee】. When directly @-mentioned, if the employee judges “this isn't my context to reply”, it delegates to the selected stand-in to answer (sent under this employee's identity, marked “replied by X”). **Organizational delegation with upstream escalation**: delegation can span multiple levels (A→B→C…); any level that judges “not its context” and has **no downstream to delegate to (or the whole downstream chain also declines)** **escalates back up**, until the topmost (the @-mentioned host) answers as the final fallback — so no one is left unanswered and no one is forced to answer out of scope. A depth cap (≤4) and cycle protection (A→B→A cannot loop) are enforced. Unlike full handoff, it only delegates/escalates when a level judges the context is out of scope.
  **Demo pack**: `tools/agents-standin-demo.json` ships test digital employees (“总前台 → 一线-人力分线 → 人力专员”), import them and @总前台 to observe down-chain delegation (HR questions) and up-chain escalation (non-HR questions, with the desk answering as fallback).
