...
- **Orchestration adapts to the built-in producer skills (implemented)**: both the prompt and the reusable-skills section now name them — “PPT / presentation → reference the built-in `pptx_deck`”, “Word → `docx_report` / `docx_gongwen` / `docx_notice`” — and explicitly forbid inventing text-only prompt shells for capabilities that already exist; the preview’s `DetectDeliveryGap` / `DetectHollowDeliverySkill` raise a `deliveryWarning` early. Deliverable detection is unified as **explicit-format-first, generic-Chinese-last**, so “make a PPT with a comparison table” is classified as a presentation deliverable (not Excel) and no longer misreports a missing `xlsx_` skill.
