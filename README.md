...
- ✅ **内置文档 / 表格 / 演示文稿 / PDF 生成（开箱即用）**：`dotnet` 技能随程序集内置，无需手工导入——Word 三个场景（`docx_gongwen` 公文 / `docx_notice` 通知公告 / `docx_report` 工作报告）、Excel（`xlsx_book` 工作簿）、PowerPoint（`pptx_deck` 演示文稿）与 PDF（`pdf_doc` 打印级文档）。无需 Node / Python / 外部解释器，与平台其它技能同一条 Roslyn 执行链路（docx/pptx/xlsx 用 DocumentFormat.OpenXml，pdf 用 PDFsharp）。
  - `pptx_deck` 按页面类型组织（封面 / 目录 / 章节分隔 / 内容 / 两栏 / 表格 / 指标卡 / 引言 / 图片 / 图表 / 小结 / 结束页），支持六套主题预设或自定义配色与字体、16:9 宽屏、除封面外每页页码徽标、每页可带演讲者备注；图表（柱 / 折线 / 饼 / 环形）**渲成图片嵌入**而非发 ChartPart（避开 DrawingML 图表兼容性风险，与内置 docx 技能同口径）；图表字体会**自动挑选含中文字形的那一款**（不会因选中纯拉丁字体把中文画成空心方框），并在返回信息里报出 `chartFont`；**文字自适应**：每页标题与正文都按**真实字形**量高后自动缩字号，要点页 / 小结 / 目录与表格过长会**自动分页**（标题带「（n/m）」，不丢条目；卡片类装不下会截断并在 `warnings` 说明），出稿自检新增 `textOverflow`（文字放不进自己的框），因此不会交出“文字压出框”的稿子；**每页配图**：任意出图位置（封面 / 章节页 / 配图页 / 要点页）写 `imageQuery`（英文关键词效果最好）即**先查团队图库**（命中则用本地图片，**无需署名**）、未命中再回落 Wikimedia Commons 取**真照片**（免密钥、自由许可；`content` 页带图会自动变成「文左图右」）；**模型看不到图库里有什么**，所以当它给的关键词命中置信度不够（或只配到通用网图）时，技能会自动拿**本页文字**（标题 / 正文，截断 120 字）再查一次图库，**谁分高用谁、自有素材优先** —— 页面里写着“· 刘佳俊”这类人名 / 产品名时能配上对应的自有图（实测：关键词“员工 颁奖 舞台”配到 0.68 的不相干合影，本页文字“刘佳俊”直查 0.88）；用了网络照片会**自动在稿末附一页「图片来源」**（CC 许可的署名要求，多到放不下会自动分页且一条不漏）并把元数据回显在 `images[]`；取不到图 / 无外网会降级为题图并在 `warnings` 说明（配上图时不会假报降级），不会让整份稿子失败（端点可用 `imageSearchApi` 或环境变量 `AGUI_PHOTO_API` 指向镜像 / 代理；`AGUI_IMAGE_SOURCE=library` 可彻底不出网）。
...
**各技能怎么配图**：`pptx_deck` 命中图库后仍可在库内无果时回落 Wikimedia，`docx_*` / `pdf_doc` 只走图库
（`{"image":{"imageQuery":"关键词"}}` / `{"type":"image","imageQuery":"…"}`，取不到就**跳过该图**并在 `warnings` 说明，
不让整篇稿子失败；PDF 只嵌 PNG/JPEG，命中项格式不对会明确告知而不是生成坏文件）。
...
配图顺序（PPT）：图库命中 → 未命中回落 Wikimedia → 都没有降级为题图；`AGUI_IMAGE_SOURCE=library` 可让它**彻底不出网**。
