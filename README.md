...
- ✅ **运行超时按任务复杂度自适应**：所有运行不再共用一条固定预算——先按触发消息估出任务量级（交付物格式 / 页数 / 字数 / 条目数 / 多步措辞 / 附件规模 / 是否向下指派），再在 `StreamTimeoutMinutes`（默认 5 分钟）之上乘倍率（常规 ×1.5、复杂 ×2、繁重 ×3），并以 `MaxRunTimeoutMinutes`（默认 30 分钟）兜上界。**三处连带放宽**，避开“主预算放开了、某一层先掉链子”的短板：内置文档类技能预算同倍率放宽、客户端（本机桥）技能隧道等待上界由硬编码 180 秒改为 180×倍率、模型 HTTP 网络兜底改为 `MaxRunTimeoutMinutes+5`。指派链（计划→逐步→递归补查→交付兑底）在自适应预算上再 ×3。两条不变量：**只放宽不收紧**（任何档位都不低于你原本配的值）、**确定性**（审批 / 桥接恢复用同一份触发上下文重算，预算不会莫名变化）。预算被放宽时日志会写明档位与依据（`运行超时预算按任务复杂度放宽：… Heavy（分 8，×3，5→15 分钟，依据：交付物格式、41 页…）`），可直接回答“这次为什么跑这么久”；可在「管理员 → 执行参数」热改或一键关闭（`complexityTimeouts.enabled=false`）。**同一档位也定档「自动放行上限」**：`MaxAutoApprovedRounds`（默认 30）退化为简单档基准值 / 关闭自适应时的兜底，启用后实际上限 = `max(基准值, 常规 40 / 复杂 60 / 繁重 100)` ——时间放开了、轮数不该先到，本是一件事。详见 `docs/execution-configuration.md` §A.1 与 `docs/agent-execution-algorithm.md` §2.2
- ✅ **审批轮数分两道线 + 终止前先保产物**：`MaxInteractionRounds`（默认 5 → **15**）现在只数“真的打断了用户”的那一轮——**已同意技能 / 批量批准这类自动放行不再计入**，而是走新配置 `MaxAutoApprovedRounds`（默认 30，且自 **1.0.163** 起**按任务复杂度定档**：简单档＝基准值 30，常规 40 / 复杂 60 / 繁重 100，**只放宽不收紧**，见上一条）。旧实现把两者混在一个计数器里，导致一次正常的长生成（文档技能天然要被反复调用：出正文 → 逐页配图 → 改）会在第 5 次调用就被判超限杀掉——实测踩到 **40 页 PPT 已生成到磁盘，却因“交互恢复超过最大轮数（5）”终止运行、产物没挂上消息**，用户只看到空白兜底文案。现在**终止前会先把已产出的产物回挂到消息**（与其它收尾路径同一原则：先保证用户能拿到东西，再谈为什么中断）。**同一原则现已覆盖崩溃路径**：审批恢复抛异常时也**先回挂本轮已产出的产物**再收尾——实测浅色版 PPT 已落盘、却因恢复请求被模型拒（`HTTP 400 reasoning_content must be passed back`）而中断，用户只看到“回复说已出稿”却拿不到文件。另外：一个模型轮次里可提出**多个**需审批的调用，现在**全部收下、全部回应**（一张卡、一个决定作用于全组，卡文写明“本轮共 N 项待确认”）——日志里三次 400 全部紧跟“一轮两条审批、只回一条”的那一步，而单审批恢复从未失败。可在「管理员 → 执行参数」热改。
- ✅ **向量化两池隔离（交互不被后台饿死）**：embedding 服务在 CPU 上只有一条推理通道，**客户端并发度基本等于排队长度**；知识库单片 4096 字 ≈ 35 秒，多个后台任务并发入库会把交互检索排到 60 秒超时（日志表现为“语义记忆检索失败”，看着像服务挂了）。现在把向量化并发拆成两个互不抢占的池子：**交互池**（回复前记忆 / 知识库 / 图谱检索，默认 3 并发）与**后台池**（记忆写入 / 导入 / 知识库入库，默认 1 并发），两池之和 = 旧版总并发（4），**不增加**对 embedding 服务的压力、只是重新分配优先权。等不到槽位时：交互侧**降级**（本次不注入这段可选上下文），后台侧跳过本条由下次补上。实现 `EmbeddingGates`，被 `AgentMessageMemory` 与 `KnowledgeBaseCatalog` 共用。详见 `docs/agent-execution-algorithm.md` §2.4
- ✅ **记忆向量化的输入预算（治“检索莫名失败”）**：日志里的 `语义记忆检索失败：HttpClient.Timeout of 60 seconds elapsing` 看着像 embedding 服务挂了，实际是**排队超时**。实测（4 核 CPU + bge-m3）embedding 耗时由**输入长度**主导，约 **8.5ms/字符**：6 字 0.3 秒、**1500 字 12.7 秒**；而写入侧原本把**整条消息原文不限长**送进 embedding（一条 5000 字回复 ≈ 40 秒+），检索侧 query 上限 2000 字（≈17 秒/条）× 一轮约 4 条检索 → 合计 60 秒+，正好撞穿超时。现在：检索 query 上限 **2000 → 500**（语义检索只需主题含义，反而提高信噪比）、新增写入侧上限 `MaxWriteChars=800`（**同时作用于入库文本与向量**，避免“检索命中却内容对不上”）、新增 `EmbeddingConnectTimeoutSeconds=5`（把「连不上」与「排队中」拆开，服务不可用时秒级判死而非白等满总预算）、新增 `SlowEmbeddingWarnSeconds=10`（单次超时就记 WARN 并带输入字符数）。**为什么不靠并发/并行**（实测）：服务端并行槽 1→4 只快 **~9%**（瓶颈是 4 个核在算），批量化对长文本**完全无效**（1500 字×4：批量 49.6s vs 串行 48.7s）——这是“算术量”问题，不是“并发度”问题。详见 `docs/agent-execution-algorithm.md` §2.4
- ✅ **提示词装配预算（分段 + 总闸门 + 截断可观测）**：模型上下文由若干段拼成（群历史 / 历史附件回喂 / 当前附件 / 记忆·知识库注入），此前各段上限散在两处、各自独立，且**截断完全静默**——用户能看到文字被截断，却查不出是哪一层截的。现在收拢为一份可配预算（appsettings 顶层 `PromptBudget`），单项上限保持宽松（历史单条 500→**4000** 字符、历史附件回喂 24K→**96K**、附件单文件 12K→**40K**、合计 60K→**200K**），规模由**总闸门**（默认 200000 字符）兜住：超预算按降级顺序从尾部裁（`history` → `history_attachments` → `attachments`），**当前消息与系统提示永不截断**；**只要有截断就记一条 WARN**，写明用了多少 / 上限多少 / 丢了什么。为什么不贴窗口定？模型（DeepSeek-V4.1-Flash）官方推荐 `context_window = 1M tokens`，而实测本平台单次 prompt 仅 3.5K~17.5K tokens（窗口的 0.35%~1.75%），真正的约束是成本与延迟（每轮重建上下文 → 每次调用都重付 prefill）。同时给正式回复**显式输出预算**（`Agents:MaxOutputTokens` 默认 16000；此前不设，等于把长度交给提供方默认值，而实测 reasoning 占 completion 的 **57%~79%**），并接好 `Agents:ReasoningEffort`（`none/low/medium/high`；默认不启用，因取值口径需实测）。详见 `docs/agent-execution-algorithm.md` §2.3
- ✅ **一键组织编排（`数字员工 → 一键编排`）**：输入一句话需求 → **SSE 流式**生成方案（`POST /ag-ui/agents/orchestrate/stream`，逐 token 实时显示生成过程 + 实时统计已见岗位 / 技能）→ 预览数字员工组织架构 + 各岗位技能 + 岗位连接（向下指派 / 向上提升 / 汇报中继）→ 确认后整体落库（`POST /ag-ui/agents/orchestrate/apply`，先**对服务端执行的技能冒烟自测 + 大模型自动修复**（最多 3 次）再落库，幂等不部分写入）并返回 `smoke[]`，UI 在创建后的通知中心展示；可勾选「同时创建客服知聚」把方案数字员工作为客服团队一键建群上线；方案会为每个岗位自动挑好**记忆拟人类型（memoryProfile）**并**自动成对指派 / 提升连接**——生成的团队既能向下派活、也能向上报问题，不会退化成单向提升链；并从提示词与预览校验两侧**保证交付闭环**：要 PPT / 演示文稿时提示模型<b>直接引用内置 `pptx_deck`</b>（Word 同理引 `docx_report` / `docx_gongwen` / `docx_notice`）而不是自造空壳技能，方案里没人能产出用户要的文件格式时预览会给出 `deliveryWarning`；交付物判定**明确格式词优先、中文泛称靠后**，因此「做份 PPT，含一张对比表格」判为演示文稿交付而非 Excel
...
- ✅ **内置文档 / 表格 / 演示文稿 / PDF 生成（开箱即用）**：`dotnet` 技能随程序集内置，无需手工导入——Word 三个场景（`docx_gongwen` 公文 / `docx_notice` 通知公告 / `docx_report` 工作报告）、Excel（`xlsx_book` 工作簿）、PowerPoint（`pptx_deck` 演示文稿）与 PDF（`pdf_doc` 打印级文档）。无需 Node / Python / 外部解释器，与平台其它技能同一条 Roslyn 执行链路（docx/pptx/xlsx 用 DocumentFormat.OpenXml，pdf 用 PDFsharp）。
  - `pptx_deck` 按页面类型组织（封面 / 目录 / 章节分隔 / 内容 / 两栏 / 表格 / 指标卡 / 引言 / 图片 / 图表 / 小结 / 结束页），支持六套主题预设或自定义配色与字体、16:9 宽屏、除封面外每页页码徽标、每页可带演讲者备注；图表（柱 / 折线 / 饼 / 环形）**渲成图片嵌入**而非发 ChartPart（避开 DrawingML 图表兼容性风险，与内置 docx 技能同口径）；图表字体会**自动挑选含中文字形的那一款**（不会因选中纯拉丁字体把中文画成空心方框），并在返回信息里报出 `chartFont`；**文字自适应**：每页标题与正文都按**真实字形**量高后自动缩字号，要点页 / 小结 / 目录与表格过长会**自动分页**（标题带「（n/m）」，不丢条目；卡片类装不下会截断并在 `warnings` 说明），出稿自检新增 `textOverflow`（文字放不进自己的框），因此不会交出“文字压出框”的稿子；**每页配图**：任意出图位置（封面 / 章节页 / 配图页 / 要点页）写 `imageQuery`（英文关键词效果最好）即**从团队图库检索**（命中则用本地图片，**无需署名、也完全不依赖外网**；`content` 页带图会自动变成「文左图右」）；**模型看不到图库里有什么**，所以当它给的关键词命中置信度不够时，技能会自动拿**本页文字**（标题 / 正文，截断 120 字）再查一次图库，**谁分高用谁** —— 页面里写着“· 刘佳俊”这类人名 / 产品名时能配上对应的自有图（实测：关键词“员工 颁奖 舞台”配到 0.68 的不相干合影，本页文字“刘佳俊”直查 0.88）；取不到图会降级为题图并在 `warnings` 说明（配上图时不会假报降级），不会让整份稿子失败。**联网取图（Wikimedia Commons）自 1.0.164 起暂时下线**：默认只查图库，只有**显式**写 `imageSource=auto|network`（入参或环境变量 `AGUI_IMAGE_SOURCE`）才会出网（端点覆盖 `imageSearchApi` / `AGUI_PHOTO_API` 仍在，恢复不必改代码）；因命中均为自有素材，正常出稿**不再出现「图片来源」署名页**。
  - `xlsx_book` 支持多工作表、列定义与数字格式（千分位 / 货币 / 百分比 / 日期）、**公式优先**（合计行自动生成 SUM 等公式，跨表引用用 `表名!`）、冻结窗格与自动筛选、合计行上框线（会计式），并遵循财务配色惯例（硬编码输入＝蓝、公式＝黑、跨表公式＝绿）；另有 `analyze` 模式读取已有 .xlsx 并回报结构摘要。
  - `pdf_doc` 由**设计令牌**驱动：多种文档类型（report / proposal / resume / academic / minimal / editorial / magazine / terminal）各带封面版式与强调色，内容块体系（标题 / 正文 / 列表 / callout / 引用 / 表格 / 图片 / 图表 / 代码 / 分隔线 / 目录 / 分页）可直接给结构化 blocks，也可把整篇 Markdown 交给它重排（REFORMAT 路由）；A4/Letter、页码页脚、中文字体自动探测并**子集嵌入**（字体说明见 `tools/pdf-skills/README.md`）。
- ✅ **交付物在线查看（无需下载）**：消息里带 docx / xlsx / pptx（含 odt/ods/odp/rtf）附件的，卡片旁多一个「👁 在线查看」——服务端用 LibreOffice 转成 PDF 后**内联**返回（`GET /ag-ui/preview/{attachmentId}`），前端在宽幅弹窗 iframe 里直接阅读，不用装本地 Office；PDF 附件直通不转换。权限与下载**完全相同**（未登录 401 / 非成员 403 / 已撤回不可读 / 不存在 404），因此不存在绕过群权限的读取口子。转换结果按「附件 ID + 源文件指纹」缓存（保留 7 天），同一份文档二次打开只要几十毫秒（实测首次：docx ≈2.5s、xlsx ≈1.7s、4.4MB/16 页 pptx ≈5.3s；二次均 ≈15ms）；转换并发串行、单次 120s 超时，服务端未装 LibreOffice 时降级为 503 并提示可下载后用本地应用打开，不影响其它功能。
- ✅ **技能库试运行也回档产物**：在技能库点 ▶ 手动试运行一份生成类技能（如内置 `docx_report`），结果弹窗里多出「📦 本次产出」区，列出本次生出的文件，可**直接下载**、也可**在线查看**（复用上面同一套阅读器）——以前结果只有一段文本，文件确实生成了却拿不到。服务端把结果里的 `produce_file` 标记按与聊天回档**同一实现**校验后入库（扩展名白名单 / 非空 / 产物尺寸上限），并登记归属：**产出者本人可读**，他人仍 403；每次重跑先清空上一次的产出。本机桥 / 客户端执行的试运行不回档（产物在用户自己机器上，服务端读不到）。

  四类产物均带 `produce_file` 标记 → 网关自动登记为附件并挂到对话，**前端可直接下载**。可由 `Agents:BuiltinDocxSkills` / `Agents:BuiltinPptxSkills` / `Agents:BuiltinXlsxSkills` / `Agents:BuiltinPdfSkills` 分别关闭播种（详见各技能目录下的 README）。
...
- ✅ **全局智能检索**：顶栏「🔎 全局搜索」跨知聚一次检索<b>消息 / 语义记忆 / 知识库</b>（`GET /ag-ui/search?q=`）；结果严格按可见性过滤（普通成员可见全员 / @提及 / 定向 / 本人发言，客服知聚成员见全部，顾客参与者仅见本人会话；记忆按成员范围，知识库按可读范围），点击消息 / 记忆直达所在知聚
- ✅ **企业合规（数据主体权利）**：**账号注销 / 数据擦除**（本人「资料 → 注销账户」需密码确认，含**注销前导出引导**；管理员「用户管理 → 彻底删除」；创建的知聚转让或解散、加入的知聚退群、发言记忆与个人知识库清除、现存群发言匿名化、数字员工 / 技能孤儿清理、账号 + 会话 + TOTP 删除，最后一名超级管理员防删）+ **「导出我的数据」可携权导出**（`GET /ag-ui/account/export`）+ **孤儿定义运营盘点页签**（`GET /ag-ui/admin/orphans`：已注销 Owner 的数字员工 / 技能 接管或安全删除）+ **审计日志检索 / CSV 导出 / 独立表持久化**（`/ag-ui/admin/audit(.csv)`；管理员控制台「审计日志」页签；数据库模式 `agui_audit` 表）（详见 `docs/enterprise-compliance.md`）
...
- ✅ **用户分组与按组授权（细粒度访问控制）**：管理员控制台「用户分组」把平台账号编组（`ug_xxx`，与“群聊知聚”不同概念）；资源可指定「允许访问的用户组」白名单——**数字员工** `AllowedGroupIds`（未配置=全员；配置后仅命中的组成员可看到 / 单聊 / 拉入建群；服务端四关口拦截，403 `AGENT_PERMISSION_DENIED`）、**技能库** `AllowedUserGroupIds`（受限技能对非授权者隐藏且不可挂载）；知识库已有“群级共享”与之互补。创建者与系统管理员始终放行（详见 `docs/RBAC.md` §6）
- ✅ **记忆拟人类型（按类型召回）**：每个数字员工可在「编辑 → 记忆与权限」配置自己的记忆特征——五档拟人预设（广记型 / 深记型 / 难录入型 / 存得住想不起型 / 快速遗忘型）+ 口吻模式 + 人设 + TopK / 阈值微调；调取群 / 个人记忆时按类型实际执行抽取（条数、相似度阈值、回忆提示分支、近期窗口），深记型本人发言自动刻为「重要」记忆、快速遗忘型在开启自动遗忘时本人普通发言保留更短。未配置 = 沿用平台全局，完全向后兼容（机制、字段与调试见 `docs/memory-personality.md`）；**组织构建自动配型**——网页一键组织编排与内置组织架构构建师产稿时会按每个岗位职责自动挑选合适的记忆拟人预设，落库的新数字员工即自带匹配其岗位的记忆类型（见 §4.4）
...
| `AGENTS_EXECUTION_*` | 不限 | 网关**执行参数**（`Agents:Execution`：时序/重试/TTL、阶段开关、`ExecutionOrder`、**复杂度自适应超时 `ComplexityTimeouts`**）**默认与代码一致，无需容器配置**——首选在运行期于控制台「管理员 → 执行参数」页热改并持久化（`executionRuntime`，重启恢复）。若确要容器启动即用非默认值：`docker-compose.yml` `web.environment`（Docker 不透传未知键）需<b>显式补一行</b>，如 `Agents__Execution__StreamTimeoutMinutes: "8"`、`Agents__Execution__ComplexityTimeouts__Enabled: "false"`、`Agents__Execution__EnableOrgRoute: "false"`。角色级按单数字员工在「编辑 → 执行阶段」关闭桥接/交接/组织路由。字段/默认/可配边界见 `docs/execution-configuration.md` |
...
| `PROMPT_BUDGET_*` / `AGENTS_MAX_OUTPUT_TOKENS` / `AGENTS_REASONING_EFFORT` | 见文档 | **提示词装配预算**（顶层 `PromptBudget`：总闸门 / 历史窗口与单条截断 / 历史附件回喂 / 附件注入长度 / 附图数量）与**正式回复的输出预算、推理力度**。代码默认已按 1M 窗口放宽，无需配置；Docker 不透传未知键，要非默认值须在 `docker-compose.yml` 的 `web.environment` 显式补行（如 `PromptBudget__MaxTotalChars: "400000"`、`Agents__MaxOutputTokens: "32000"`）。字段与分层降级语义见 `docs/agent-execution-algorithm.md` §2.3 |
...
| 文档在线查看 | `GET /ag-ui/preview/{attachmentId}` | 把 docx / xlsx / pptx（含 odt/ods/odp/rtf）用服务端 LibreOffice 转成 PDF 后**内联**返回（`application/pdf`，不设 `Content-Disposition`），供前端弹窗 iframe 直接阅读；`.pdf` 直通不转换。**权限与下载完全同一套校验**（未登录 401 / 非成员 403 / 附件不存在 404）；不支持的类型 400、服务端缺 LibreOffice 503、转换失败 500（错误码 `DOCUMENT_PREVIEW_FAILED`）。转换结果按「附件 ID + 源文件指纹」缓存，同一文档二次打开毫秒级 |
...
- **导出全部数据**（`📦 导出全部数据`）：账号（含密码哈希 / 盐，导入后原密码可直接登录）+ 智能体定义与触发规则 + 全部群的成员 / 话题 / 消息（含撤回 / 附件 / 思考内容）+ 附件与头像文件，打包为 zip（`manifest.json` 另含 `skills` 技能库全量与 `orgTeams` 组织覆盖簿记两段，保证挂载的技能实体与“同一 teamKey 覆盖只留最新”语义一并还原）+ `files/`；AI 分身与技能目标子代理不导出
- **导入数据**（`📥 导入数据`）：上传 zip → 后端返回<b>账号 / 智能体 / 技能库</b>存在性检查与群清单 → 勾选要恢复的群 → 执行导入：账号按 username 检查（缺失创建并保留密码哈希；**已存在则更新资料**——昵称 / 头像 / 个人记忆开关，密码保留不覆盖），智能体按 agentId 检查（缺失创建，含桥接配置与触发规则），消息发送者 / 提及 / 可见列表按账号映射重写，附件与头像文件按原 `attachmentId` 还原；**同时补齐技能库缺失实体**（已存在不覆盖，目标侧沿用其技能库定义）与缺失的<b>组织覆盖簿记 key</b>（保证“同 key 覆盖只留最新”跨机一致）；导入的群使用新 groupId 避免冲突，消息直接落库不触发智能体
- **初始化（清空一切）**（`🗑 初始化`，危险操作区，需输入「确认」）：删除全部数据（账号 / 智能体 / 群 / 消息 / 附件 / 语义记忆 / 会话 / 配置），并将<b>技能库复位为 appsettings 种子技能、组织覆盖簿记一并清空</b>，所有已登录端立即失效，并<b>清空浏览器缓存</b>回到登录页；再次进入系统时自动弹出<b>模型配置</b>界面
...
管理 API：`POST/GET/DELETE /ag-ui/kb`、`PUT /ag-ui/kb/{kbId}`（库设置：检索严格度）、`POST/DELETE /ag-ui/kb/{kbId}/documents(/docId)`
（仅创建者可管理；系统级知识库只读）；依赖向量存储与 embedding（与语义记忆同一套：pgvector / sqlite-vec + llama / http embedding），
不可用时文档入库返回明确错误。

**检索严格度可按库调**（`PUT /ag-ui/kb/{kbId}`，界面在「📚 管理知识库」每行的 `⚙️`）：门槛存在**知识库**上，
检索时**库设了就以库为准**，没设就沿用全局 `Agents:Memory:MinScore`（默认 0.25）——所以未设置的库行为完全不变。
为何要按库调：文档是**短条目 / 目录型**时通用提问得分普遍偏高（该收紧），是**长篇论述**时一句话提问得分偏低（该放松）。
实测（真实文档 + bge-m3）：无关提问 **0.31~0.38**、真实命中 **0.41~0.71**（分离度比图库窄，所以档位间距也小）。
三档：宽松 0.15 / 标准 0.25（= 平台默认）/ 严格 0.40。图库侧同机制（档位 0.5 / 0.6 / 0.72，实测噪声 ≤0.55、真实命中 0.62~0.88）。

**图库（Image Library，文档技能配图的图片来源）**：用户上传图片的集合，让**配图不依赖外网**（内网部署里 Wikimedia 通常不可达）。
与知识库同源但存的是图片：上传走同一个 `/ag-ui/upload`（图片扩展名与 20MB 限制一样），
登记进图库后由**视觉模型自动生成中文描述**（`Agents:VisionEnabled`，失败则用文件名 + 手填描述），
描述 + 标签 + 文件名一起向量化存入同一张向量表（GroupId 约定 `img:{LibId}`，`sender_type='img'`，一张图一个向量），
同样**不参与群记忆检索**（两个 store 都排除 `img` 行）。
图片文件落 `data/images/{libId}/{assetId}{ext}`（与上传件同属一个数据卷）。
`POST /ag-ui/image-libs`、`POST/DELETE /ag-ui/image-libs/{libId}/assets(/assetId)`、`GET .../assets/{assetId}/raw`；
权限与知识库一致（创建者可写、系统级仅管理员可写、群级共享只读、管理员全量）。
**技能怎么用上**：平台在调用文档技能前登记一个「检索范围句柄」（范围 = 触发者本人可读的图库；
岗位**绑定了图库就只用绑定的那几个**——绑定的库全部不存在时不注入，**不会**放宽成本人全部图库）并注入技能入参，
技能经回环 `POST /ag-ui/images/search` 取回**本地文件路径**直接嵌入；
技能**只带句柄、不自报图库 ID**（入参是模型生成的，自报就能越权读别人的图），
该端点只对**进程内自令牌**返回路径（`AGUI_SELF_BASE` / `AGUI_SELF_TOKEN`，启动时注入），登录用户只拿元数据。
覆盖的入口：聊天触发（含单聊 / 知聚 / 编排）、技能库「试运行」、桌面宿主直跑 —— 三条路径都会注入，
避免「聊天里能配图、界面上手动试运行却一张图都没有」这种难查的入口差异。
**各技能怎么配图（1.0.164 起统一为图库单源）**：`pptx_deck` / `docx_*` / `pdf_doc` 现在**都只查团队图库、不再联网**
（`imageQuery` / `{"image":{"imageQuery":"…"}}`，取不到就跳过该图并在 `warnings` 说明，不让整篇稿子失败；
PDF 只嵌 PNG/JPEG，命中项格式不对会明确告知而不是生成坏文件）。
联网取图（Wikimedia Commons）**暂时下线**：默认 `library`，且**只有显式**写 `imageSource=auto|network`（入参或环境变量 `AGUI_IMAGE_SOURCE`）
才会出网——代码与端点覆盖（`imageSearchApi` / `AGUI_PHOTO_API`）全部保留，恢复只改一个入参/环境变量。
因为命中是自有素材，正常出稿**不再出现「图片来源」署名页**。

**三者共用的“配得准”机制**（1.0.147 起 docx / pdf 与 pptx 同口径）：① 请求里**显式带 `minScore: 0.6`**
（不传就是平台默认 0.25，实测无意义关键词能到 0.44~0.55 —— 等于不筛）；② 关键词命中**不够确定**（< 0.78）或没命中时，
再用**上下文候选**（图自己的 caption → 最近的标题 → 前面最近的文字块）**逐个查、取高分者**，命中即停。
候选**分开查**而不是拼成一串：实测把“caption + 标题 + 正文”拼成一句，关键人名被稀释（人名直查 0.88 → 拼串 0.63）。
三者都回显 `images[{query,fileName,score}]`，配错了能自己查。
配图顺序（一律）：团队图库命中 → 未命中降级为自动题图（并在 `warnings` 说明）；默认 `AGUI_IMAGE_SOURCE=library` **彻底不出网**。

**检索严格度可按库调**（`PUT /ag-ui/image-libs/{libId}`，界面在「🖼️ 管理图库」每行的 `⚙️`）：
严格度存在**图库**上，检索时**库设了就以库为准**（比技能传的 0.6 更松或更紧都行），没设就沿用技能的值。
为何要按库调：描述是**短人名 / 标签**时通用关键词得分普遍偏高（该收紧，否则容易配错），
描述是**长句**时标题式查询得分偏低（实测 0.62，该放松才配得上）。三档：宽松 0.5 / 标准 0.6（推荐）/ 严格 0.72。

**库设置弹窗里带「🔍 试检索」**：用**当前选中的档位**当场试一次，直接看到会召回哪些（含分数与片段预览），
调档位不再靠猜；换档位会自动重测。知识库侧对应 `POST /ag-ui/kb/{kbId}/search`，也只回片段预览。
...
#### 技能库（可复用技能：prompt / shell / http / dotnet / org_deploy）
...
- **记忆可视化**：用户菜单 →「记忆管理」——各群记忆统计（条数 / 最近时间 / 已过期数）、按群 / 关键词浏览条目（时间 / 发送者 / 级别 / 内容）、单条<b>分级 / 删除</b>、按群<b>遗忘</b>；权限校验：仅可查看 / 治理自己所在群的记忆（非成员 403）。**可管理范围按角色分层**：平台管理员（Admin / SuperAdmin）可跨全部知聚查看 / 治理任意记忆；知聚群主 / 群管理员可在其知聚内<b>整群</b>治理（对他人记忆分级 / 删除、整群遗忘、向该群导入记忆）；普通成员仅可治理<b>本人</b>发言；Operator（只读运维）不因平台角色获得记忆治理特权。记忆管理界面按所选知聚提示当前遗忘范围（整群 / 仅本人）
...
1554 个 Hub 用例 + 6 个 SDK 用例覆盖：群生命周期、权限控制、订阅与快照、可见性扇出（all/mentioned/private）、撤回、踢出/退群、在线状态联动、智能体触发规则（含语境触发与**群内触发方式覆盖角色默认**、**分身在线暂停**）、MSAGENT 网关流式回灌（mock + 增量/累计文本兼容 + 语境发言决策 + 群内触发模式生效）、**人机交互（审批中断产出 ToolApprovalRequestContent、仅触发者可决策、批准后恢复同一会话执行工具并回灌、`approveAll` 一次性批准）**、DeepSeek/API Key 配置解析、**用户管理（注册/登录/改密/资料/头像同步/个人记忆开关/令牌/WS·SSE 鉴权/多设备会话 / TOTP 二次验证）**、**智能体运行时管理（动态目录增删改 + 头像 + 私密智能体权限 + 智能体级差异化审批 + 角色交接 relay + 市场导入 + HTTP 管理 API）**、**AI 分身（启用/停用/触发方式修改/公开群跟随/同步）**、**语义记忆（pgvector 写入/检索/私密群隔离/解散删记忆/个人记忆/时间线回放/混合 BM25 重排/沉淀知识库/记忆拟人类型按类型召回）**、**话题（新建/删除/按消息新建/清空话题记录/跨话题主题关联）**、**审批与治理（细粒度 RBAC、操作审计日志检索与 CSV 导出、TOTP）**、**企业合规 · 账号注销 / 数据擦除（管理员删除与自助注销：知聚转让·解散·退群、发言记忆与个人知识库清除、现存群发言匿名化与数字员工/技能孤儿清理、账号 / 会话 / TOTP 删除、最后一名超级管理员防删）**、**平台角色（SuperAdmin/Admin/Operator/User：`POST /ag-ui/admin/roles/{userId}` 授予 / 回收、不能自降级、Admin 不能管理角色）**、**一键组织编排（非流式 + SSE 流式方案生成、apply 全量校验后落库、客服知聚一键建群）**、**客服知聚会话隔离（顾客参与者非成员、各自独立会话、发现 / 进入接口）**、**技能库（prompt/shell/http 三类、shell/http 仅管理员、ClientRunner、自然语言生成）**、**内网隧道（`NativeTunnel__Token`、逐 agent 令牌、限流、`ClientToolTunnelRequireApproval` 批准路径）**、**复杂度自适应运行超时（档位估算确定性 + 倍率换算 + 上界夹紧 + “只放宽不收紧”不变量 + 复杂度联动内置技能预算 / 用户技能不受影响 + 环境预算跨 await 传播 + 自动放行上限定档与基准值兜底）**、**编排与定时（多步工作流 pipeline、重复性定时任务）**、**富媒体附件（图片多选 / 语音 audio 类别 / 画布标注，音频不注入文本上下文）**、**白标品牌与嵌入（6.4：公开读取 + 管理员保存 + 非法主色 / 危险 Logo 拒绝 + 越权 403）**、**配置治理（6.3：管理员在线写入 / 持久化 + 非法值 400 / 非管理员 403）**、**记忆跨实例同步（2.3：导出 / 导入 round-trip + 幂等去重 + sinceMs 增量）**、**持久化（JSON 快照 round-trip + 全应用重启恢复）**、**PostgreSQL 存储**（群/成员/话题/消息分页/撤回/原地修改/用户/触发规则/扩展区 round-trip + 全应用 PG 重启恢复，需本地 PG 测试库，`AGUI_PG_TEST_CONN` 覆盖连接串）、**MySQL 存储**（同上 11 例，需本地 MySQL 8.0.13+，`AGUI_MYSQL_TEST_CONN` 覆盖连接串）、**SQLite 存储**（同上 11 例，单文件零部署本机即跑），未配置数据库时对应用例自动跳过；以及真实 Kestrel 上的 HTTP + WebSocket 全流程集成测试。
