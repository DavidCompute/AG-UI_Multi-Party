...
- ✅ **企业合规（数据主体权利）**：**账号注销 / 数据擦除**（本人「资料 → 注销账户」需密码确认；管理员「用户管理 → 彻底删除」；创建的知聚转让或解散、加入的知聚退群、发言记忆与个人知识库清除、现存群发言匿名化、数字员工 / 技能孤儿清理、账号 + 会话 + TOTP 删除，最后一名超级管理员防删）+ **「导出我的数据」可携权导出**（`GET /ag-ui/account/export`：账号 / 知聚 / 本人发言 / 记忆 / 知识库 / 数字员工与技能定义）+ **审计日志检索与 CSV 导出且跨重启持久化**（`/ag-ui/admin/audit` 过滤查询、`/ag-ui/admin/audit.csv` RFC 4180 + UTF-8 BOM 全量导出；管理员控制台「审计日志」页签）（详见 `docs/enterprise-compliance.md`）
