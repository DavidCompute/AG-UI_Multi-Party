...
- ✅ **运行超时按任务复杂度自适应**：所有运行不再共用一条固定预算——先按触发消息估出任务量级（交付物格式 / 页数 / 字数 / 条目数 / 多步措辞 / 附件规模 / 是否向下指派），再在 `StreamTimeoutMinutes`（默认 5 分钟）之上乘倍率（常规 ×1.5、复杂 ×2、繁重 ×3），并以 `MaxRunTimeoutMinutes`（默认 30 分钟）兜上界。**三处连带放宽**，避开“主预算放开了、某一层先掉链子”的短板：内置文档类技能预算同倍率放宽、客户端（本机桥）技能隧道等待上界由硬编码 180 秒改为 180×倍率、模型 HTTP 网络兜底改为 `MaxRunTimeoutMinutes+5`。指派链（计划→逐步→递归补查→交付兑底）在自适应预算上再 ×3。两条不变量：**只放宽不收紧**（任何档位都不低于你原本配的值）、**确定性**（审批 / 桥接恢复用同一份触发上下文重算，预算不会莫名变化）。预算被放宽时日志会写明档位与依据（`运行超时预算按任务复杂度放宽：… Heavy（分 8，×3，5→15 分钟，依据：交付物格式、41 页…）`），可直接回答“这次为什么跑这么久”；可在「管理员 → 执行参数」热改或一键关闭（`complexityTimeouts.enabled=false`）。详见 `docs/execution-configuration.md` §A.1 与 `docs/agent-execution-algorithm.md` §2.2
- ✅ **审批轮数分两道线 + 终止前先保产物**：`MaxInteractionRounds`（默认 5 → **15**）现在只数“真的打断了用户”的那一轮——**已同意技能 / 批量批准这类自动放行不再计入**，而是走新配置 `MaxAutoApprovedRounds`（默认 30）。旧实现把两者混在一个计数器里，导致一次正常的长生成（文档技能天然要被反复调用：出正文 → 逐页配图 → 改）会在第 5 次调用就被判超限杀掉——实测踩到 **40 页 PPT 已生成到磁盘，却因“交互恢复超过最大轮数（5）”终止运行、产物没挂上消息**，用户只看到空白兜底文案。现在**终止前会先把已产出的产物回挂到消息**（与其它收尾路径同一原则：先保证用户能拿到东西，再谈为什么中断）。可在「管理员 → 执行参数」热改。
