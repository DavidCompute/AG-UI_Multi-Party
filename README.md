...
- ✅ **内置文档 / 表格 / 演示文稿 / PDF 生成（开箱即用）**：`dotnet` 技能随程序集内置，无需手工导入——Word 三个场景（`docx_gongwen` 公文 / `docx_notice` 通知公告 / `docx_report` 工作报告）、Excel（`xlsx_book` 工作簿）、PowerPoint（`pptx_deck` 演示文稿）与 PDF（`pdf_doc` 打印级文档）。无需 Node / Python / 外部解释器，与平台其它技能同一条 Roslyn 执行链路（docx/pptx/xlsx 用 DocumentFormat.OpenXml，pdf 用 PDFsharp）。
  - `pptx_deck` 按页面类型组织（封面 / 目录 / 章节分隔 / 内容 / 两栏 / 表格 / 指标卡 / 引言 / 图片 / 图表 / 小结 / 结束页），支持六套主题预设或自定义配色与字体、16:9 宽屏、除封面外每页页码徽标、每页可带演讲者备注；图表（柱 / 折线 / 饼 / 环形）**渲成图片嵌入**而非发 ChartPart（避开 DrawingML 图表兼容性风险，与内置 docx 技能同口径）；图表字体会**自动挑选含中文字形的那一款**（不会因选中纯拉丁字体把中文画成空心方框），并在返回信息里报出 `chartFont`。
