...
- ✅ **记忆拟人类型（按类型召回）**：每个数字员工可在「编辑 → 记忆与权限」配置自己的记忆特征——五档拟人预设（广记型 / 深记型 / 难录入型 / 存得住想不起型 / 快速遗忘型）+ 口吻模式 + 人设 + TopK / 阈值微调；调取群 / 个人记忆时按类型实际执行抽取（条数、相似度阈值、回忆提示分支、近期窗口），深记型本人发言自动刻为「重要」记忆、快速遗忘型在开启自动遗忘时本人普通发言保留更短。未配置 = 沿用平台全局，完全向后兼容（机制、字段与调试见 `docs/memory-personality.md`）
