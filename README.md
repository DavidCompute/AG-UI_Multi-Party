...
- 端口可用 `--local-port`（设为 `0` 关闭回环发现）；网页来源用 `--allowed-origin` 做 CORS 白名单（不配则仅同源可读）。
- **回环发现的“加密/安全”考虑**：回环端点只监听 `127.0.0.1`（流量不出本机），返回的也仅是机器名等<b>非敏感</b>标识，本机读取足够安全；浏览器的回环读取通过 CORS 白名单 + 仅本机可达来防护跨源/伪造。若仍需<b>传输加密</b>，可用 `--local-https` 让回环改用 HTTPS（自签证书）——但浏览器对自签证书默认不信任，需把该证书加入信任库后才能让页面自动读取，故默认用 HTTP。
