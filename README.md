...
桥启动时（默认）在 `127.0.0.1:17321` 起一个<b>极简回环发现服务</b>，只提供 `GET /ag-ui/bridge/info`，返回非敏感的机器标识：

```json
{ "client": "AIBOOK", "agentScope": "*", "online": true }
```

网页「修改资料 → 本机执行客户端 → 💻 自动发现」读取它并自动填入 `client`。因此：浏览器与桥<b>同一台机器</b>（如 Docker 在 localhost、桥也在 localhost）时，无需手填机器名即完成“本机执行客户端”绑定；
不同机器上各自的浏览器会读到各自机器的桥标识，实现<b>按发起请求的客户端路由</b>。

- 端口可用 `--local-port`（设为 `0` 关闭回环发现）。
- **回环发现的“加密/安全”考虑**：回环端点只监听 `127.0.0.1`（流量不出本机），返回的也仅是机器名等<b>非敏感</b>标识，故对回环 GET 直接放开跨源读取（`Access-Control-Allow-Origin: *`），保证同机浏览器的“自动发现”能读到；端口仅本机可达、不返回任何令牌/密钥，因此这样足够安全。若仍需<b>传输加密</b>，可用 `--local-https` 让回环改用 HTTPS（自签证书）——但浏览器对自签证书默认不信任，需把该证书加入信任库后才能让页面自动读取，故默认用 HTTP。
- 回环发现仅用于“读取本机机器标识”，<b>不含</b>令牌等机密；隧道的认证仍由 `--tunnel-token` / 逐 agent 令牌完成。
