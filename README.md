...
- ✅ **内置文档 / 演示文稿生成（开箱即用）**：`dotnet` 技能随程序集内置，无需手工导入——Word 三个场景（`docx_gongwen` 公文 / `docx_notice` 通知公告 / `docx_report` 工作报告）与 PowerPoint（`pptx_deck` 演示文稿）。**纯 .NET 实现**（DocumentFormat.OpenXml），不依赖 Node / Python / 外部解释器，与平台其它技能同一条 Roslyn 执行链路。`pptx_deck` 按页面类型组织（封面 / 目录 / 章节分隔 / 内容 / 两栏 / 表格 / 指标卡 / 引言 / 图片 / 图表 / 小结 / 结束页），支持六套主题预设或自定义配色与字体、16:9 宽屏、除封面外每页页码徽标、每页可带演讲者备注；图表（柱 / 折线 / 饼 / 环形）**渲成图片嵌入**而非发 ChartPart（避开 DrawingML 图表兼容性风险，与内置 docx 技能同口径）。产物带 `produce_file` 标记 → 网关自动登记为附件并挂到对话，**前端可直接下载**。可由 `Agents:BuiltinDocxSkills` / `Agents:BuiltinPptxSkills` 关闭播种（详见 `tools/pptx-skills/README.md`）
