...
- **Agent context scope**: in a support circle, an agent's context window includes, besides the group-visible messages (`Visibility=all`), <b>the triggering customer's own isolated session</b> (that customer's questions + staff messages directed back to that customer), but never leaks another customer's private chat. Ordinary circles still only inject `Visibility=all` messages.
...
<b>Support-circle broadcast scope</b>: when a support-staff member (agent) is typing, the event goes not only to the other staff members but also to <b>every customer participant who has entered</b>; when a customer participant is typing, it is broadcast to the support staff only (customers stay invisible to one another, matching message isolation). Ordinary circles still broadcast typing to group members only.
...
> `latestMessages` elements may actually also carry **optional fields** (legacy clients may ignore them): `senderNickname`, `replyToMessageId`, `attachments`, `reasoning`, `mentions`, `mentionAll`, `topicId`, `senderType` (authoritative, returned on history / topic / snapshot / paged messages: `Agent` = digital employee or AI twin (`twin_*`), `User` = human; the client uses it to decide whether “regenerate” applies, instead of guessing from an `agent_` prefix).
...
|One-click orchestration (apply/persist)|`POST /ag-ui/agents/orchestrate/apply`|Persists a confirmed plan: body `{ title, agents[], skills[], createSupportCircle?, supportCircleName? }`. Each `agents[]` entry mirrors the upsert body fields (`agentId/nickname/description/instructions/triggerMode/skillIds/assignmentIds/escalationAgentId/relayToAgentId` plus an optional `memoryProfile` memory-personality preset, auto-picked per role by the orchestration / org-architect generator). <b>Connections are auto-paired on persist</b>: when a plan only sets upward escalation (`escalationAgentId`) and misses the downward assignment rosters, the server merges each leader's direct subordinates (who escalate to it) into its `assignmentIds` at parse and at apply — deduped, order-preserving, additive-only — so every landed org carries both assign-down and escalate-up links. Validates and <b>smoke-tests everything first</b>, then writes skills + digital employees + connections **atomically (all-or-nothing)**；`shell`/`http`/`dotnet` skills require admin。Smoke scope: `prompt` skills run once against a representative sample input；`http` skills are only config-linted（JSON `method` / `url` validity，no outbound call）；server-executed `shell` skills are blind-run only when `Agents.SkillAutoTestServerShell`（env `AGENTS_SKILL_AUTOTEST_SHELL`，default `true`）is on；client / tunnel kinds and anything without an executable body are skipped。Smoke failures are auto-repaired by the model（<b>up to 3 attempts</b>）。Besides the collections，the response carries a `smoke[]` array —— `{skillId, skipped, ok, attempts, repaired, lastError}`（the frontend surfaces it in the notification center after creation）. <b>Auto-dedup on duplicate names (plan A)</b>: if a generated digital-employee agentId or skillId clashes with an existing one in the library (or duplicates another in the plan), it is auto-renamed with `_2`/`_3` and saved anyway — no full failure and no overwriting existing assets — and in-plan references are re-mapped automatically (employees' SkillIds, assignment/escalation AssignmentIds / EscalationAgentId / RelayToAgentId, support-circle members, and the IDs returned to the frontend). When `createSupportCircle=true` it additionally builds a **support circle** (§2.1.1) with those employees as the support team and registers their trigger rules, returning `{ applied, title, agents[], skills[], supportCircleGroupId }`；login required|
...
|memoryProfile|object?|**Memory personality type（1.0.121+）**：`{memoryType, styleMode?, personaCard?, topK?, personalTopK?, minScore?, personalMinScore?}`；`memoryType` ∈ `broad` / `deep` / `slowToLearn` / `cueDependent` / `fastForgetting`（missing / unknown = not configured，following the global defaults），`styleMode` is `recall` / `digest`，numeric tuning is clamped out-of-range（TopK 1~24 / 1~16，thresholds 0.05~0.92）。It decides per-employee tuning of group / personal memory recall（hit count / threshold / recall-cue branch / recency window）and a mild write-side adjustment for the employee's own posts（see `docs/memory-personality.md`）|
...
|allowedGroupIds|string[]?|**Fine-grained access authorization (2.9)**：platform user-group ids (`ug_xxx`) allowed to see / direct-chat / add this employee into a circle。Missing / empty = no group restriction (backward compatible)；when set，outside the creator and system admins only users in one of the listed groups may use it；privacy still overrides (an allowlist does not open up a private agent)。 Enforced server-side at catalog list / direct chat / group create / member add。The skill library mirrors this as `allowedUserGroupIds` (see `docs/RBAC.md` §6)|

**Export / import**: the "📤 Export All" / per-row "Export" of agent management exports the configuration as JSON (`{version: 1, agents:[…]}`，fields as above，sensitive tokens and ownerId are not exported)，and the frontend "📥 Import" reads the JSON and calls `POST /ag-ui/agents` one by one to create them（owned by the current logged-in user；on agentId conflict the ID is auto-renamed without overwriting）。Additionally there is a **full data package** API（administrator）：`GET /ag-ui/export` exports accounts（including password hash / salt）+ agent definitions and trigger rules + groups / topics / messages + attachments as a zip；`POST /ag-ui/import/preview` uploads a zip and returns an existence check of accounts / agents and a group manifest；`POST /ag-ui/import` executes per `selectedGroupIds`（accounts are auto-completed by username，agents by agentId；message senders / mentions / visible lists are rewritten according to the account mapping；attachments are restored）。The export zip's `manifest.json` also carries a `skills` section (the skill library) and an `orgTeams` section (the org-overwrite ledger)；importing back-fills missing skill records and missing org-team ledger keys；AI twins and skill-target sub-agents are not exported.

**Skill library（reusable skill catalog，Hub extension）**: skills are a global, reusable catalog (`AgentSkillDefinition`) that any digital employee can mount by ID via `AgentDefinition.SkillDefIds`（referenced as tool names for the model）。Skills come in four kinds — `prompt` / `shell` / `http` and `dotnet` (C#)：a `dotnet` skill body is C# source exposing `public static string Run(string input)`，compiled at run time with Roslyn and executed on the server。`shell` / `http` / `dotnet`（can run arbitrary commands / external requests / C# source）**can only be created / edited / deleted by system admins**，ordinary users may create pure `prompt` skills；the kinds are also protected from being demoted or re-typed by non-admins。However **running an existing `dotnet` skill is open to all logged-in users**——any user may trial-run or have an agent invoke a server-side `dotnet` skill，or，for a `client`-executed one，approve and let the local native bridge run it（the browser cannot compile C#；client dotnet runs via the bridge's `DotnetRunner`）；`shell`/`http` trial-runs remain admin-only。Server-side `dotnet` skills execute inside the Hub process in a collectible `AssemblyLoadContext` with a constrained reference allowlist + `AllowUnsafe=false` + timeout / output caps。A skill has an `executionLocation` of `server` (default) or `client`。Client-executed shell skills run on the **local host** — either the frontend browser or, when an intranet bridge is online, over the reverse tunnel (§4.5). When `executionLocation=client` and no `clientRunner` is provided, the server auto-generates one (`{"kind":"shell","command":…,"cwd":".","timeoutSec":30}`) for shell skills; `clientRunner` can also be supplied explicitly. `shell` and `client`-executed skills are always `requiresApproval=true`.
There is also a controlled <b>`org_deploy` (org write-to-store)</b> kind: it has no runnable body, can only be created / edited / deleted by system admins in the skill library (digital employees cannot self-create it at run time, like `dotnet`), and when mounted on a digital employee (e.g. `org_architect`) it becomes that employee's “deploy the final org plan / overwrite, keeping only the newest version per team key” action (through the single shared apply engine, admin-gated); a regular user only reaches a preview / “needs admin approval” note and never writes to the store.
...
|Generate skill from natural language|`POST /ag-ui/skills/generate`|Input a one-sentence requirement（`{ request, preferClient? }`）；the LLM produces a structured skill definition `{ skillId, name, kind, description, body, executionLocation, clientRunner, requiresApproval }` for the frontend to fill the form（no hand-written commands / JSON）；the produced kind follows the same rule——`dotnet` only for system admins（others get prompt / http / shell）|
|Create skill|`POST /ag-ui/skills`|body per `SkillDefHttpRequest`；`shell`/`http`/`dotnet` only for admins；skillId empty → auto-generated ASCII tool id|
|Update skill|`PUT /ag-ui/skills/{skillId}`|Owner or admin only；`shell`/`http`/`dotnet` cannot be created or re-typed by non-admins|
|Delete skill|`DELETE /ag-ui/skills/{skillId}`|Owner or admin only；system skills（ownerId=null）read-only|
|Suggest a test input|`POST /ag-ui/skills/{skillId}/suggest`|Before a trial run, auto-suggest a <b>representative example input</b> derived by the model from the skill's description / body；login required|
|Run / test skill|`POST /ag-ui/skills/{skillId}/run`|Manual test of a skill（no-approval channel）；the frontend typically calls `/suggest` first to pre-fill an example input。`prompt` only for owner/admin；`shell`/`http` and system skills admin-only，avoid arbitrary command execution；**`dotnet`（including system skills）is open to all logged-in users**——server-executed, or client-executed over the native bridge `DotnetRunner`（a browser cannot compile C#）。**The response carries `attachments[]`: the files this run produced**（built-in docx / xlsx / pptx / pdf skills write to server disk, and previously the user got only text and could not obtain the document）——the server parses the `produce_file` marker out of `result` and registers those files as attachments using **the same implementation as the chat recall path**（extension allow-list / non-empty / produced-file size cap），recording ownership so that **only the producer can read them**（`/ag-ui/files` and `/ag-ui/preview` both allow the producer; everyone else still gets 403）。The frontend turns this into a “⬇ Download” and “👁 View online” entry。Native-bridge / client-executed paths are **not** collected（those files live on the user's own machine and the server cannot read them）|
...
4. **Contextual trigger（contextual）**：the server decides based on the context of recent messages（`Agents:ContextMaxMessages` messages，default 10）；on a hit，the corresponding agent is awakened
   - The contextual speaking decision（`ShouldSpeakAsync`）is the same as ordinary circles — it uses only the group-visible messages（`Visibility=all`）as its decision context；however in a **support circle**, the actual <b>reply context window</b>（`BuildUserMessageAsync`）additionally includes the triggering customer's own isolated session（their questions + staff messages directed back to that customer）besides the group-visible messages, but never leaks another customer's private chat（see §2.1.1）.
   - Implementation：this verdict is a **small decision** — it always uses a **non-reasoning** model（`Agents:DecisionModel`；empty = the regular model；**deliberately unaffected by `Agents:ThinkingMode`**）, reads the model's `logprobs` to compute **P(speak)** and compares it against `Agents:DecisionMinProbability`（default 0.3）. A single verdict therefore takes roughly 100–250ms and does not get slower in thinking mode. Only when no probabilities are available does it fall back to text（first word only；unrecognized = do not speak）.
