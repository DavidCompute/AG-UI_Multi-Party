...
|One-click orchestration (apply/persist)|`POST /ag-ui/agents/orchestrate/apply`|Persists a confirmed plan: body `{ title, agents[], skills[], createSupportCircle?, supportCircleName? }`。Validates everything first, then writes skills + digital employees + connections **atomically (all-or-nothing)**；`shell`/`http` skills require admin. <b>Auto-dedup on duplicate names (plan A)</b>: if a generated digital-employee agentId or skillId clashes with an existing one in the library (or duplicates another in the plan), it is auto-renamed with `_2`/`_3` and saved anyway — no full failure and no overwriting existing assets — and in-plan references are re-mapped automatically (employees' SkillIds, assignment/escalation AssignmentIds / EscalationAgentId / RelayToAgentId, support-circle members, and the IDs returned to the frontend). When `createSupportCircle=true` it additionally builds a **support circle** (§2.1.1) with those employees as the support team and registers their trigger rules, returning `{ applied, title, agents[], skills[], supportCircleGroupId }`；login required|
