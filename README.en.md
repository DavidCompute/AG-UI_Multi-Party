...
- ✅ **Org-plan drafts (multi-round editing, built-in tool)**: organization-building digital employees (those mounting the `org_design` skill, e.g. an "Org Architect") get draft tools `org_draft_save / org_draft_load / org_draft_list / org_draft_delete`. They persist the organization / customer-service / office-team plan being refined as a per-group **draft keyed by an English slug** (versions auto-increment), so that even **across topics / after the plan scrolls out of the recent-message window** they can reload the old draft and keep editing it, then overwrite the same slug to bump the version. Drafts are scoped per group and persisted in the `orgDrafts` extension section (Postgres writes to `agui_sections` / Memory writes a JSON snapshot), surviving restarts. **A draft is always just "plan text" and never auto-commits the real organization / skills** — real construction still requires an admin's manual confirmation in “一键编排”.
...
src/AguiGroupChat.Agents/        # MSAGENT agent gateway: AgentGateway (IAgentGateway implementation), AgentCatalog, MemoryContextProvider (RAG injection), KnowledgeBaseCatalog (knowledge base: document chunk vectors + retrieval), TwinService (user twin + ITwinAgentSync hook), IAgentDefinitionStore (private agent ownership), MockChatClient, skills (AgentSkillCall inter-agent invocation); embedding abstraction (IEmbeddingProvider: HTTP / LLamaSharp local model); built-in tool set (Tools/: calculator / unit_converter / group_memory_search / read_attachment / web_search / read_url / OrgDraftTools org-plan draft save/load/list/delete)
...
**Org-plan draft tools (agent-scoped)**: organization-building digital employees that mount the `org_design` skill (e.g. an "Org Architect") additionally get these — persist / reload per-group organization-plan drafts keyed by slug so plans stay editable across topics and long gaps; a draft is only plan text and never auto-commits the real org (see §User Management「一键编排」):

| Tool | Description | Approval |
|---|---|---|
| `org_draft_save(slug,title,content)` | Save the current plan as a per-group draft (same slug overwrites & bumps the version) | No |
| `org_draft_load(slug)` | Fetch the group's saved plan by slug (to keep editing an older draft) | No |
| `org_draft_list()` | List the group's saved plan drafts (slug / title / version) | No |
| `org_draft_delete(slug)` | Delete a group's plan draft (irreversible) | No |
...
#### Skill library (reusable skills: prompt / shell / http / dotnet)
