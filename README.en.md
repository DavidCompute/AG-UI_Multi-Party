...
- ✅ **Image understanding (vision)**: when a group message carries an image attachment, that turn is automatically routed to a dedicated **vision model**, which sees the image (fed as base64 multimodal content) and can describe / answer about it; plain text-only messages keep the normal / thinking model, unchanged. Under DeepSeek the default vision model is `deepseek-v4-flash-vision-exp` (override with `Agents:VisionModel`; `Agents:VisionEnabled` defaults to on as the master switch); the `mock` provider has no vision, so image messages just behave as text. The image is read inline from the attachment store as bytes + MIME — no extra file is saved; the shared member-approval / attach-image workflow is unchanged.
- ✅ **Two separate approval counters, and products are kept on termination**: `MaxInteractionRounds` (default 5 → **15**) now counts only rounds that actually **interrupted a user** — auto-releases (already-approved skills / batch approval) no longer count and are bounded separately by the new `MaxAutoApprovedRounds` (default 30, **tiered by task complexity since 1.0.163**: simplest tier = baseline 30, standard 40 / complex 60 / heavy 100, **widen-only**; see the previous bullet). Mixing both into one counter killed legitimate long generations (document skills are naturally called repeatedly: body → per-page illustrations → edits) at the fifth call — measured: a **40-page deck was already on disk when the run terminated with "interaction rounds exceeded (5)", the file was never attached** and the user only saw the blank fallback. Termination now **re-attaches already-produced products first**, following the same rule as every other teardown path: secure the user's artifacts before explaining the interruption. **The same rule now covers the crash path**: when an approval resume throws, already-produced artifacts are **re-attached before the message is ended** — measured case: a light-themed deck was on disk but the run died because the model rejected the resume request (`HTTP 400 reasoning_content must be passed back`), so the user saw “generated” with no file. Also, one model turn may request **several** approvals; they are now **all collected and all answered** (one card, one decision for the whole group, stating “N items pending”) — all three 400s in the log immediately followed a turn with two approvals answered only once, while every single-approval resume succeeded. Hot-tunable under Admin → Execution Parameters.
- ✅ **Embedding pools are isolated (interactive work is no longer starved by background work)**: the embedding service has a single inference lane on CPU, so **client concurrency effectively equals queue length**; a 4096-char knowledge chunk takes ~35 s, and several background ingestions in parallel push interactive retrieval past the 60 s timeout — which surfaces as a misleading "semantic memory retrieval failed". Embedding concurrency is now split into two non-preempting pools: **interactive** (pre-reply memory / knowledge / graph retrieval, 3 concurrent by default) and **background** (memory writes / imports / knowledge ingestion, 1 by default). Their sum equals the previous total (4), so pressure on the embedding service is unchanged — only priority is redistributed. When no slot is available the interactive side **degrades** (that optional context is skipped) and the background side skips the item for a later pass. Implemented as `EmbeddingGates`, shared by `AgentMessageMemory` and `KnowledgeBaseCatalog`. See `docs/agent-execution-algorithm.md` §2.4
- ✅ **Memory embedding input budget (fixes "retrieval mysteriously fails")**: the log line `语义记忆检索失败：HttpClient.Timeout of 60 seconds elapsing` looks like the embedding service died, but it is really a **queue timeout**. Measured (4-core CPU + bge-m3) embedding cost is dominated by **input length**, roughly **8.5 ms per character**: 6 chars = 0.3 s, **1500 chars = 12.7 s**. The write path used to send the **entire message verbatim** into embedding (a 5000-char reply ≈ 40 s+), while the retrieval query cap was 2000 chars (≈17 s each) across ~4 retrievals per turn — totalling 60 s+ and landing exactly on the timeout. Now: query cap **2000 → 500** (semantic retrieval only needs the topic, and it improves signal-to-noise), a new write cap `MaxWriteChars=800` (**applied to both the stored text and the vector**, so hits never mismatch their content), a new `EmbeddingConnectTimeoutSeconds=5` (separates "cannot connect" from "queued": an unreachable service fails in seconds instead of burning the whole budget), and `SlowEmbeddingWarnSeconds=10` (a slow single call logs a WARN with its char count). **Why not more concurrency/parallelism** (measured): raising the server's parallel slots from 1 to 4 sped up 8 concurrent short texts by only **~9%** (the bottleneck is the 4 cores doing the math), and batching is **useless for long text** (1500 chars × 4: 49.6 s batched vs 48.7 s serial). This is an arithmetic-volume problem, not a concurrency one. See `docs/agent-execution-algorithm.md` §2.4
- ✅ **Prompt assembly budget (sectioned + total gate + truncation observability)**: the model context is assembled from several sections (group history / re-inlined historical attachments / current attachments / memory·knowledge injection). Their caps used to be scattered across two places, independent of each other, and **truncation was entirely silent** — you could see text truncated but not tell which layer did it. They are now one configurable budget (top-level `PromptBudget` in appsettings) whose per-section caps stay generous (single history message 500→**4000** chars, historical attachment re-inline 24K→**96K**, attachment per file 12K→**40K**, total 60K→**200K**) while the **total gate** (default 200000 chars) bounds the size: when over budget, sections are trimmed from the tail following the downgrade order (`history` → `history_attachments` → `attachments`), and the **current message plus system prompt are never truncated**; **any truncation emits a WARN** stating how much was used / the cap / what was dropped. Why not size it to the window? The model (DeepSeek-V4.1-Flash) officially recommends a `context_window` of 1M tokens, while this platform's measured prompt per call is only 3.5K–17.5K tokens (0.35%–1.75% of the window) — the real constraint is cost and latency (the context is rebuilt every turn, so prefill is re-paid on every call). The official-reply path now also gets an **explicit output budget** (`Agents:MaxOutputTokens`, default 16000; previously unset, leaving length to the provider default while reasoning accounts for **57%–79%** of measured completion), and `Agents:ReasoningEffort` (`none/low/medium/high`) is wired up but off by default, since its value semantics still need measuring. See `docs/agent-execution-algorithm.md` §2.3
- ✅ **Run timeouts adapt to task complexity**: runs no longer share one fixed budget — the trigger message is first scored for task magnitude (deliverable format / page count / word count / item count / multi-step wording / attachment size / whether the role fans out downward), then the budget multiplies on top of `StreamTimeoutMinutes` (default 5 min) — ×1.5 standard, ×2 complex, ×3 heavy — capped by `MaxRunTimeoutMinutes` (default 30 min). **Three budgets widen together** so a narrowed inner limit cannot defeat the widened run budget: the built-in document-skill budget scales by the same multiplier, the client (native-bridge) skill tunnel wait ceiling goes from a hard-coded 180 s to 180 × multiplier, and the model HTTP network fallback becomes `MaxRunTimeoutMinutes + 5` instead of a fixed 15 min. The assignment chain (plan → step-by-step → recursive follow-up → delivery fallback) gets another ×3 on top, still capped. Two invariants: **widen-only, never shrink** (no tier ever goes below the values you configured) and **deterministic** (approval / bridge resumes recompute from the same trigger context, so a budget never changes mid-run). When a budget is actually widened the log states the tier and the evidence (`运行超时预算按任务复杂度放宽：… Heavy（分 8，×3，5→15 分钟，依据：交付物格式、41 页…）`), which directly answers "why did this run take so long?"; tune it live under Admin → Execution Parameters, or switch it off (`complexityTimeouts.enabled=false`). **The same tier also sets the auto-approval limit**: `MaxAutoApprovedRounds` (30) becomes the simplest-tier baseline / fallback when adaptive is off, and the effective limit is `max(baseline, standard 40 / complex 60 / heavy 100)` — if time is widened, the round count should not run out first; it is one and the same decision. See `docs/execution-configuration.md` §A.1 and `docs/agent-execution-algorithm.md` §2.2
- ✅ **One-click organization orchestration (`数字员工 → 一键编排`)**: type a one-line requirement → **SSE streaming** generates a plan (`POST /ag-ui/agents/orchestrate/stream`, displaying generation token-by-token in real time + live counts of seen roles / skills) → preview the digital-employee org chart + per-role skills + connections (downward assignment / upward escalation / relay) → confirm and apply to storage (`POST /ag-ui/agents/orchestrate/apply`, which first **smoke-tests the server-executed skills and auto-repairs failures with the model — up to 3 attempts** — before saving, atomically with no partial writes) and returns a `smoke[]`; the UI surfaces the results in the notification center after creation; you can check "同时创建客服知聚" to turn the planned digital employees into a customer-service team group in one click; each plan auto-picks a **memory-personality preset** per role and **auto-pairs assignment / escalation links** — generated teams can both hand work down and raise problems up, never degrading into a one-way escalation chain; plans are also kept **delivery-closed-loop** from both sides — the prompt tells the model to **reuse the built-in `pptx_deck`** for PPT / presentation requests (and `docx_report` / `docx_gongwen` / `docx_notice` for Word) instead of inventing hollow skills, while the preview emits a `deliveryWarning` when nobody in the plan can produce the requested file format; deliverable detection is **explicit-format-first, generic-Chinese-last**, so “make a PPT with a comparison table” is classified as a presentation deliverable, not Excel
- ✅ **Skill system (skill library)**: reusable skills come in `prompt` / `shell` / `http` / `dotnet` (C#) — `shell` / `http` / `dotnet` (can run arbitrary commands / external requests / C# source) can **only be created / edited / deleted by system admins**, ordinary users may create pure prompt skills; but **running an existing `dotnet` skill is open to all logged-in users** (trial-run a server dotnet skill, or have a client-executed one run via the native bridge — no admin needed just to run). Local (client-executed) shell / dotnet skills run on the local machine via the native bridge / tunnel (`ClientRunner` auto-generated; the browser cannot compile C#, so client dotnet runs on the local bridge `DotnetRunner` after the triggerer approves); the skill library supports search (frontend filter) and bulk delete; its "trial run" **auto-suggests a representative example input first** (`POST /ag-ui/skills/{skillId}/suggest`); `POST /ag-ui/skills/generate` creates a skill definition from natural language (only yields `dotnet` for system admins)
- ✅ **Built-in document / spreadsheet / presentation / PDF generation (works out of the box)**: `dotnet` skills shipped inside the assembly — no manual import — covering three Word scenarios (`docx_gongwen` official document / `docx_notice` notice / `docx_report` report), Excel (`xlsx_book` workbook), PowerPoint (`pptx_deck`) and PDF (`pdf_doc` print-ready documents). No Node / Python / external interpreter, on the same Roslyn execution path as every other skill (docx / pptx / xlsx use DocumentFormat.OpenXml, pdf uses PDFsharp).
  - `pptx_deck` organises a deck by slide type (cover / toc / section / content / two-column / table / KPI / quote / image / chart / summary / end), supports six theme presets or explicit colors and fonts, 16:9 widescreen, a page badge on every page but the cover, and optional speaker notes per slide; charts (bar / line / pie / doughnut) are **rendered to PNG and embedded as pictures** rather than emitted as ChartParts (the same call the Word skills make to avoid DrawingML chart compatibility risk), and the chart font is **picked for actual CJK glyph coverage** (so Chinese never degrades into hollow notdef boxes) with the chosen face reported as `chartFont`; text auto-fits: every title and body is measured with **real glyph metrics** and shrunk to fit, over-long bullet pages / summaries / TOC and tables **paginate automatically** (titles carry “(n/m)”, no item is dropped; fixed cards trim and say so in `warnings`), and the post-generation QA now reports `textOverflow` — so a deck whose text spills out of its boxes is never handed to users; **per-page photos**: put `imageQuery` (English keywords work best) on any image slot (cover / section / image / content page) and the **team image library is searched** (a local file, **no attribution needed, and no internet at all**; a `content` page with a photo automatically becomes image-right text-left). If nothing matches, the slide degrades to generated art with a `warnings` entry instead of failing the whole deck. **Web image search (Wikimedia Commons) is temporarily retired as of 1.0.164**: only the library is searched by default, and egress requires *explicitly* setting `imageSource=auto|network` (input field or `AGUI_IMAGE_SOURCE`); the endpoint overrides (`imageSearchApi` / `AGUI_PHOTO_API`) are still there, so restoring it needs no code change. Because every hit is your own asset, a deck no longer gains an **“Image credits” page**.
  - `xlsx_book` supports multiple worksheets, column definitions and number formats (thousands separator / currency / percent / date), **formulas first** (totals rows generate SUM and friends, cross-sheet references use `Sheet!`), frozen panes and autofilter, an accounting rule above totals, and the usual financial color convention (hard-coded input = blue, formula = black, cross-sheet formula = green); an `analyze` mode reads an existing .xlsx and reports a structural summary.
  - `pdf_doc` is driven by **design tokens**: several document types (report / proposal / resume / academic / minimal / editorial / magazine / terminal) each with their own cover layout and accent color, plus a block system (heading / paragraph / list / callout / quote / table / image / chart / code / divider / toc / page break) — feed it structured `blocks`, or hand it a whole Markdown document to re-typeset (the REFORMAT route). A4 / Letter, page numbers and footers, and a Chinese font that is auto-detected and **subset-embedded** (font details in `tools/pdf-skills/README.md`).

  All four outputs carry the `produce_file` marker → the gateway registers them as attachments on the conversation, **downloadable straight from the frontend**. Seeding can be turned off per format with `Agents:BuiltinDocxSkills` / `Agents:BuiltinPptxSkills` / `Agents:BuiltinXlsxSkills` / `Agents:BuiltinPdfSkills` (see the README in each skills directory).
- ✅ **View deliverables online, without downloading**: messages carrying docx / xlsx / pptx (plus odt/ods/odp/rtf) attachments gain a **👁 View online** button beside the download card — the server converts the file to PDF with LibreOffice and returns it **inline** (`GET /ag-ui/preview/{attachmentId}`), which the frontend reads in a wide modal iframe, so no local Office install is needed; PDF attachments pass straight through. Permissions are **exactly those of download** (401 unauthenticated / 403 non-member / refused once recalled / 404 missing), so no group-permission bypass exists. Artifacts are cached per “attachment ID + source fingerprint” (kept 7 days), so reopening the same document costs tens of milliseconds (measured first conversions: docx ≈2.5s, xlsx ≈1.7s, a 4.4MB/16-page pptx ≈5.3s; every cache hit ≈15ms). Conversions are serialized with a 120s timeout, and a server without LibreOffice degrades to 503 with a “download and open locally” hint instead of breaking anything else.
- ✅ **Skill trial runs also recall their output files**: clicking ▶ on a generator skill in the skill library (say the built-in `docx_report`) now shows a **📦 Produced this run** section in the result modal listing the files it wrote, each **directly downloadable** and **viewable online** (the same reader as above) — previously the result was text only, so the file existed but you could not get it. The server validates the `produce_file` marker out of the result with **the same implementation the chat recall path uses** (extension allow-list / non-empty / produced-file size cap), registers those files as attachments, and records ownership so that **only the producer can read them** while everyone else still gets 403; re-running clears the previous run's artifacts first. Native-bridge / client-executed runs are not recalled (the file lives on the user's own machine).
- ✅ **Support circle (customer-service group, `kind=support`)**: the creator pulls in humans / digital employees as the entire membership (agents who see **all conversations**); ordinary users enter as "participants" (**non-members**, no slot or member-list entry), each session isolated (customers cannot see each other; agent replies are directed to the specific customer); a customer can approve the skill executions they triggered (chat rules & org build algorithm in `docs/customer-service-and-org-builder.md`)
- ✅ **Digital-employee direct chat (`kind=direct`)**: from the <b>digital-employee list</b>, click "💬 chat" on an employee; the backend idempotently creates/reuses a one-to-one <b>private two-member group</b> for you (`POST /ag-ui/agents/direct`). Different users chatting with the same employee each get their own isolated, mutually invisible session; re-entering returns the same group (idempotent). Direct chats are private by default (`isPrivate=true`; semantic memory is only retrievable inside that private group). Inside the chat, <b>a plain (un-@) message means "talk to it" and triggers it directly</b> — no manual @ (>validated end-to-end by `tools/direct-chat-flow.mjs`, see `tools/README-playwright.md`)
- ✅ **Enterprise compliance (data-subject rights)**: **account deletion / data erasure** (self-service “Delete account” in profile with password confirmation and a **pre-deletion export guide**; admin “permanently delete” in user management; owned groups transferred or disbanded, joined groups left, message memories and personal knowledge bases erased, messages in surviving groups anonymized, orphan employee/skill cleanup, account row + sessions + TOTP removed, last-super-admin guarded) plus **“Export my data” portability** (`GET /ag-ui/account/export`) and an **orphan-definition inventory tab** (`GET /ag-ui/admin/orphans`: adopt or safely delete employees/skills whose owner account is gone) and **audit-log search, CSV export and dedicated-table persistence** (`/ag-ui/admin/audit(.csv)`; Admin Console “Audit Log” tab; `agui_audit` table on database storage) (see `docs/enterprise-compliance.md`)
...
- ✅ **User groups & group-based authorization (fine-grained access)**: the admin console's “User Groups” tab organizes platform accounts (`ug_xxx`, distinct from chat groups); resources accept an “allowed user groups” whitelist — **digital employees** `AllowedGroupIds` (unset = everyone; once set, only members of the listed groups may see / direct-chat / add to a circle; enforced at four server checkpoints with 403 `AGENT_PERMISSION_DENIED`) and **skill library** `AllowedUserGroupIds` (restricted skills are hidden and cannot be mounted by unauthorized users); knowledge bases already model group-based sharing alongside it. Creator and system admins always pass (see `docs/RBAC.md` §6)
- ✅ **Memory personality types (type-driven recall)**: each digital employee can configure its own memory characteristics under “Edit → Memory & Permissions” — five presets (Broad / Deep / Slow-to-learn / Stored-but-cue-dependent / Fast-forgetting) plus a speaking style, a persona tone and TopK / threshold tuning. Group / personal memory recall is then actually executed per type (hit count, similarity threshold, recall-cue branch, recency window); a Deep employee's own posts are auto-marked “Important”, and a Fast-forgetting employee's own ordinary posts get shorter retention when auto-forget is on. Unset = follow the platform global, fully backward compatible (mechanics, fields and debugging in `docs/memory-personality.md`); **org building auto-assigns profiles** — one-click orchestration and the built-in org architect pick a suitable memory-persona preset per role while drafting, so newly created employees ship with a memory type that matches their post (see §4.4)
...
tools/direct-chat-flow.mjs       # Playwright browser automation: digital-employee direct chat (kind=direct) start/enter/reuse + privacy/idempotency (see tools/README-playwright.md)
...
| `AGENTS_DECISION_MODEL` | empty | Model for **small decisions** (should I speak / who should this go to). **Empty (or blank) = automatically use the non-reasoning regular model, deliberately ignoring thinking mode** — these calls get only a handful of output tokens, and a reasoning model spends the whole budget on its chain of thought, leaving the text empty: the speak check then reads as "never speak" and assignment routing parses 0 downstream targets (which shows up as "only escalations, no assignments"). Set this only if you deliberately want decisions to go to a specific model |
| `AGENTS_DECISION_MIN_PROBABILITY` | `0.3` | Minimum probability (0–1) for a decision to count as "yes": the check reads the model's `logprobs` to compute P(speak) and treats anything below the threshold as "no", logging the actual probability. Default is 0.3 rather than 0.5 because "in scope but not addressed by name" — a case that legitimately deserves speaking up — measured around 0.33 |
...
| `AGENTS_SKILL_AUTOTEST_SHELL` | `true` | One-click-orchestration apply smoke test: whether to blind-run a server-executed `shell` skill once (has side effects; on by default, set `false` to disable). Maps to the option `Agents__SkillAutoTestServerShell` |
| `STORAGE_ALLOW_RECLAIM` | `false` | Storage governance: whether an admin may reclaim **unreferenced orphan attachment files** from the console's *Storage* page. Off by default. Background: clearing or deleting a topic and recalling a message **never delete attachment files** (attachments are user data), so files with no entry point left in any conversation accumulate on disk. The read-only statistics need no opt-in; even when enabled, reclaim only removes files with no reference at all that are older than the grace period. Maps to `StorageGovernance:AllowReclaim` |
| `STORAGE_ORPHAN_GRACE_HOURS` | `168` | Grace period (hours, 7 days by default) before an attachment can count as an orphan: a file just uploaded but not yet sent, or an artifact still being generated, is “temporarily unreferenced” and must not be reclaimed. Such grace-period files are still reported, not hidden: the *Storage* page shows **unreferenced (grace included)** separately from **reclaimable now**, so a “reclaimable 0” figure is never misread as “nothing is being wasted” |
...
- **RAG semantic memory by default**: `STORAGE_PROVIDER=postgres` (compose-bundled `pgvector/pgvector:pg16` image) + `MEMORY_ENABLED=true` (embedding is provided by the bundled `llama-embed`: the official llama.cpp server loading `./models/embedding.gguf`).
  ...
  At startup the HTTP embedding endpoint is **dimension-checked once** (one very short probe): if the actual dimension differs from `MEMORY_EMBEDDING_DIMENSIONS`, semantic memory is **disabled** with the fix spelled out — avoiding the **silent failure** where a wrong model makes vector columns mismatch (writes fail, retrieval always comes back empty); an unreachable endpoint only logs a WARN and does **not** disable memory, so a recoverable outage stays recoverable.
...
| View document online | `GET /ag-ui/preview/{attachmentId}` | Converts docx / xlsx / pptx (plus odt/ods/odp/rtf) to PDF with server-side LibreOffice and returns it **inline** (`application/pdf`, no `Content-Disposition`) so the frontend can read it in a modal iframe; `.pdf` passes straight through. **Exactly the same permission check as download** (401 unauthenticated / 403 non-member / 404 missing); 400 for unsupported types, 503 when the server lacks LibreOffice, 500 on conversion failure (error code `DOCUMENT_PREVIEW_FAILED`). The result is cached per “attachment ID + source fingerprint”, so reopening the same document takes milliseconds |
...
- Passwords are hashed with **PBKDF2 (SHA-256, 100,000 rounds + random salt)** and plaintext is never stored; login tokens are 32-byte random values, the real token lives only in memory and the SHA-256 hashes are stored / persisted (session storage can be swapped for Redis, a database, or JWT via `IUserStore`), surviving restarts through the extension section `agui_sections` (below).
...
**Digital-employee org chart (graphically edit task assignment / escalation)**: the top-bar「🌐 Org chart」opens a visual canvas where you drag-link each digital employee's <b>assignment (downward, multiple)</b> and <b>escalation (upward, single)</b>. <b>Double-click a digital-employee node</b> to open that employee's <b>edit form</b> (entered from the org chart, cancel / save returns to the org chart; closing the org chart then returns to the digital-employee list; entering edit from the employee-management list returns to the list). Multiple connection lines between the same pair of endpoints (e.g. A→B assignment + B→A escalation, or parallel same-direction lines) are offset with different horizontal curvature per <b>lane</b> to avoid fully overlapping. The "**Optimize dispatch**" <b>icon button</b> at each node's top-right auto-generates a "manage next-layer dispatch" guidance paragraph from that employee's <b>direct next layer</b> (`AssignmentIds`) - pick a subordinate based only on the next layer, no override, return NONE if none fits. The result can be previewed and then <b>appended to that role's Instructions</b> (`POST /ag-ui/agents/{agentId}/optimize-assignment`, login required, owner/admin only).
...
3. **Preview**: shows the generated digital employees (mounted skills / assigned subordinates / escalations / each role's auto-picked **memory-personality preset** tag) and the skill list, reviewable before applying;
4. **Confirm & apply** (`POST /ag-ui/agents/orchestrate/apply`, login required): <b>smoke-tests and validates first, then creates as a whole</b> (skills → digital employees). Each planned skill is smoke-tested once on the server; failing skills are auto-repaired by the model (<b>up to 3 attempts</b>) before saving. Smoke granularity: `prompt` skills run once against a representative input sample; `http` skills are only config-linted (JSON `method` / `url` validity — <b>no outbound call</b>); server-executed `shell` skills are blind-run only when `Agents.SkillAutoTestServerShell` (env `AGENTS_SKILL_AUTOTEST_SHELL`, default `true`) is on; client / tunnel skills and anything else are skipped. The apply response carries a `smoke[]` array — `{skillId, skipped, ok, attempts, repaired, lastError}` — which the UI surfaces in the notification center after creation. `shell`/`http`/`dotnet` skills are admin-only to create (consistent with the skill library). This happens with no partial writes. <b>Auto-dedup on duplicate names (plan A)</b>: if a generated digital-employee agentId or skillId clashes with an existing one in the library (or duplicates another in the plan), it is auto-renamed with `_2`/`_3` and saved anyway — no full failure and no overwriting existing assets — and in-plan references are re-mapped automatically (employees' SkillIds, the row/column connection AssignmentIds / EscalationAgentId / RelayToAgentId, support-circle members, and the IDs returned to the frontend). <b>Connections auto-pair at apply (backstop)</b>: when a plan only sets “subordinate → superior escalation” and misses the “superior → subordinate assignment” rosters, the direct subordinates who escalate to a leader are merged into its `AssignmentIds` before persist (deduped, order-preserving, additive-only), so the landed team is always sound in both directions;
...
- **Typing indicator**: when an agent / digital employee is typing, `GROUP_TYPING` is broadcast not only to the other agents but also to <b>every customer participant who has entered</b> (customers see real-time "agent is typing"); when a customer participant types, it is broadcast to the agents only — customers stay invisible to one another, matching message isolation. Ordinary groups still broadcast typing to group members only;
- **Agent context scope**: in a support circle, a digital employee's (agent's) context window includes, besides all group-visible messages, <b>the triggering customer's own isolated session</b> (that customer's questions + agent messages directed back to that customer, see `IsVisibleForAgentContext`), but <b>never leaks another customer's private chat</b>. Ordinary groups still only inject `Visibility=all` messages;
...
- **Export all data** (「📦 导出全部数据」): accounts (including password hash / salt; after import the original password can log in directly) + agent definitions and trigger rules + all groups' members / topics / messages (including recalled / attachments / thinking content) + attachment and avatar files, packaged as a zip (`manifest.json` + `files/`). The `manifest.json` also carries a `skills` section (the full skill library, incl. `org_design` / `org_deploy`) and an `orgTeams` section (the org-overwrite ledger: key → its batch of agents/skills), so mounted skills and the “same team key keeps only the newest” semantics survive migration; AI twins and skill-target sub-agents are not exported
- **Import data** (「📥 导入数据」): upload the zip → the backend returns <b>account / agent / skill-library existence checks</b> and the group list → check the groups to restore → run the import: accounts are checked by username (missing ones are created preserving the password hash; **existing ones get their profile updated** — nickname / avatar / personal memory switch, the password is preserved and not overwritten), agents are checked by agentId (missing ones are created, including bridge config and trigger rules), message senders / mentions / visibility lists are rewritten by account mapping, attachment and avatar files are restored by their original `attachmentId`; it <b>also back-fills missing skill-library records</b> (existing ones are not overwritten) and missing <b>org-overwrite ledger keys</b> (existing ledger entries are not overwritten). Imported groups use new groupIds to avoid conflicts, and messages are written directly to storage without triggering agents
- **Initialize (clear everything)** (「🗑 初始化」, dangerous-operations area, requires typing「确认 (confirm)」): deletes all data (accounts / agents / groups / messages / attachments / semantic memory / sessions / config), resets the skill library back to its appsettings seed skills, and clears the org-overwrite ledger; all logged-in sides become invalid immediately, and <b>clears the browser cache</b> back to the login page; the next time the system is entered, the <b>model configuration</b> UI pops up automatically
...
while `image` kinds (png / jpg / jpeg / gif / webp / bmp) are read server-side as bytes + MIME and fed inline as base64 multimodal content to a **vision model** for image understanding when enabled (in the `mock` provider or with `Agents:VisionEnabled` off, they are only treated as text); other `binary` kinds carry the filename / size / download URL for the model to be aware of.
...
**Skill-produced files** have their own **64 MB** cap plus a diagnosability guarantee: a file that is too large, has a non-whitelisted extension or is empty is not attached to the conversation, but a **Warning** is logged — previously that path was Debug-level with a silent `continue`, so “the reply says a file was produced but there is no download card” was undiagnosable in production (measured with two decks of 21 MB and 31 MB).
**Security hardening**: uploads only allow whitelisted extensions (images png/jpg/jpeg/gif/webp/bmp; audio (voice message) mp3/wav/ogg/oga/m4a/aac/flac/opus/webm; text txt/md/json/csv/xml/yaml/toml etc.; documents pdf/docx/xlsx/pptx; zip); executable / inline-rendering script categories html/js/mjs/css/svg etc. are rejected, preventing same-origin stored XSS;
...
which the user creates and uploads documents to (txt/md/json/csv and docx/xlsx/pptx/pdf, reusing the attachment text extraction) — documents are chunked (default 4096 chars/chunk + 512 overlap, tunable via `Agents:Memory:KnowledgeChunkSize` / `KnowledgeChunkOverlap`; cuts placed at line breaks or sentence-ending punctuation),
...
once registered, a **vision model writes a Chinese description** (`Agents:VisionEnabled`; on failure the filename plus the user's caption is used), and description + tags + filename are vectorised into the same vector table (GroupId convention `img:{LibId}`, `sender_type='img'`, one vector per image) — again **excluded from group-memory retrieval** (both stores filter `img` rows).
Image files live under `data/images/{libId}/{assetId}{ext}` (same data volume as uploads).
APIs: `POST /ag-ui/image-libs`, `POST/DELETE /ag-ui/image-libs/{libId}/assets(/assetId)`, `GET .../assets/{assetId}/raw`; permissions match the knowledge base (creator writes, system-level admin-writes, group sharing read-only, admins full).
**How the skills reach it**: before invoking a document skill the platform registers a *search-scope handle* (scope = the libraries the triggering user may read; if the position **binds libraries, only those are used** — and if every bound library is gone, nothing is injected rather than silently widening to all of the user's libraries) and injects it into the skill input; the skill then calls the loopback `POST /ag-ui/images/search` and gets back a **local file path** to embed.
The skill carries **only the handle, never library IDs** (its input is model-generated, so self-declared IDs would allow reading someone else's library), and that endpoint returns paths **only to the in-process self token** (`AGUI_SELF_BASE` / `AGUI_SELF_TOKEN`, injected at startup) — logged-in users get metadata only.
Injection covers every entry point: chat (direct chat / group / orchestration), the skill-library “Run” button and the desktop host — so "illustrations work in chat but not when I test-run it" cannot happen.
**Per skill (single-source library since 1.0.164)**: `pptx_deck` / `docx_*` / `pdf_doc` now **all search the team image library only, with no web search** (`{"image":{"imageQuery":"…"}}` / `{"type":"image","imageQuery":"…"}`) — a miss **skips that image** and explains itself in `warnings` instead of failing the whole document (PDF embeds PNG/JPEG only and says so rather than writing a broken file).
Web image search (Wikimedia Commons) is **temporarily retired**: the default is `library`, and egress requires *explicitly* passing `imageSource=auto|network` (input field or env var `AGUI_IMAGE_SOURCE`) — the code and the endpoint overrides (`imageSearchApi` / `AGUI_PHOTO_API`) are untouched, so restoring it is a one-value change.
Image order (always): library hit → generated art; and because hits are your own assets, decks no longer gain an “Image credits” page.

**Shared “match it right” mechanism** (docx/pdf on the same footing as pptx since 1.0.147): (1) the request carries an explicit **`minScore: 0.6`** (omitting it makes the platform fall back to 0.25, while measured nonsense keywords score 0.44-0.55 — i.e. no filtering at all); (2) when the keyword hit is **uncertain** (below 0.78) or missing, **context candidates** (the image's own caption → nearest heading → preceding text block) are queried **one by one and the highest score wins**, stopping once one is confident. Candidates are queried **separately, never concatenated** — combining them dilutes the one name that matters (0.88 for the bare name versus 0.63 combined). All three echo `images[{query,fileName,score}]` so a wrong pick can be diagnosed.
...
**The "small decision" model (`Agents:DecisionModel`) — leave it empty**: a digital employee makes two kinds of model calls — the **actual reply** (uses thinking mode, can afford a large budget) and **small decisions** (contextual triggering: "should I speak?"; org routing: "who should this go to?"), where the output budget is only a few tokens. A reasoning model spends that entire budget on its chain of thought and leaves the **text empty** — and this raises **no exception at all**: `StartsWith("YES")` is then always false, so contextually-triggered employees never speak, and assignment routing treats an empty output as "no candidate" and only escalates.
These two decision calls therefore **deliberately ignore `Agents:ThinkingMode`** and always use a non-reasoning model (`DecisionModel` → agent `Model` → global `Model`). Measured on the same decision prompt: `deepseek-flash` at budgets 8/64 → empty text; `deepseek-chat` at budget 8 → `YES`, costing 1 token.

**Value convention: blank means unset.** An empty value passed through Docker (e.g. `Agents__DecisionModel=`) binds as an **empty string**, which `??` happily accepts as "set" — that once made every decision fail with `Value cannot be an empty string. (Parameter 'model')`. Model-name resolution now falls back on blank for `DecisionModel`, `ThinkingModel` and the per-agent `Model`.

**Image understanding (vision) configuration**: `Agents:VisionEnabled` (default `true`) is the master switch, and `Agents:VisionModel` (optional model name, default empty) selects the vision model for messages that carry an image.
When `VisionModel` is empty and the provider is DeepSeek it automatically falls back to the default vision model `deepseek-v4-flash-vision-exp`; this relies on the already-configured model API key (same `Agents:ApiKey` / `DEEPSEEK_API_KEY` / `OPENAI_API_KEY`).
Both can also be overridden equivalently via the environment variables `Agents__VisionEnabled` / `Agents__VisionModel` (same mapping convention as `AGENTS__APIKEY`). The `mock` provider has no vision, so image messages behave as text.
...
A skill can be marked `ExecutionLocation = Client` (with a `ClientRunner` for `kind=http/shell/prompt/dotnet`).
When a digital employee calls such a skill, the gateway delivers a `kind=client_tool` interaction card. `http` skills are
fetched directly by the browser; `shell` and `dotnet` (C#) skills need a <b>local execution channel</b>—confirmed inline on the chat card
(no native popup), run on the local host, and the result is posted back. Because a pure browser cannot run local shell or compile C#,
the project provides two channels for `shell` / `dotnet` client skills (<b>no bridge configuration is needed in the web UI</b>):
...
- <b>Reverse NAT tunnel (recommended for real local results)</b>: the project `src/AguiGroupChat.NativeBridge` runs in
  ...
  <b>automatically pushes the shell / dotnet (C#) task down the tunnel to that machine</b> and feeds the result back—no bridge URL/token to fill in the frontend
  (the bridge's built-in `DotnetRunner` executes `kind=dotnet` tunnel tasks locally).
...
a deterministic template. The generated kind obeys the same permission rule: system admins may get `prompt` / `http` / `shell` / `dotnet`,
while everyone else only receives `prompt` / `http` / `shell`.

#### Skill library (reusable skills: prompt / shell / http / dotnet / org_deploy)
...
Beyond the **runnable** `prompt / shell / http / dotnet` kinds listed below, the library also holds a controlled <b>`org_deploy` (org write-to-store)</b> kind: it can only be created / edited / deleted by system admins in the skill library (a digital employee cannot self-create it at run time); it has no runnable body — when mounted on a digital employee (e.g. `org_architect` via its `SkillDefIds`), it becomes that employee's “deploy the final org plan / overwrite, keeping only the latest per team key” action (through the single shared apply engine, admin-gated); a regular user only gets the “needs admin approval” message and never writes to the store.

> An org role mounted on the `org_design` skill (e.g. `org_architect`) also auto-gains a **one-shot org draft** tool `org_plan_draft`: it runs the user's single-sentence brief through the same generator used by “one-click organization orchestration” (`AgentOrchestrator`) to emit a whole-team structured JSON in one pass (roles + per-role `skillIds` + skills whose `kind` is picked shell/http/prompt/dotnet and `executionLocation` server/client + connections) — avoiding hand-writing the whole team as pure-prompt skills. It only <b>drafts, never writes</b>; only after the user explicitly agrees is the same JSON committed via the existing `org_commit` (admin gate). (Full “org architect” build algorithm in `docs/customer-service-and-org-builder.md`)

> <b>Never mis-run a client / PowerShell skill as server bash</b>: the server/host executors refuse any `ExecutionLocation=Client` skill with clear guidance (run on the originating machine via NativeBridge / browser); an obviously Windows-PowerShell body landing on a <b>non-Windows</b> host (Docker/Linux server, macOS/Linux self-host) gets a clear “PowerShell environment required” message rather than a misleading `Not running in PowerShell / command not found / exit code 2`. Windows desktop self-hosting (host == the machine) runs PowerShell normally and is unaffected.

- **Four kinds**: `prompt` (prompt / flow template, no external execution), `shell` (runnable command / script, runs in the dedicated sandbox `data/skillruns`), `http` (calls an external HTTP endpoint; body is a JSON config `{method,url,headers,body}`; supports `$QUERY` / `{{query}}` placeholders), `dotnet` (a body of C# source exposing `public static string Run(string input)`, compiled at run time with Roslyn and executed). Any digital employee mounts a reference via its "reusable skills (skill library)" checkbox (`SkillDefIds`); the model auto-invokes them when needed.
- **`dotnet` (C#) execution & permissions**: `kind=dotnet` shares the same <b>privileged bucket</b> as `shell` / `http` for **create / edit / delete — system admins only** (ordinary users may only create `prompt` skills and cannot retype into `dotnet`), and `POST /ag-ui/skills/generate` yields `dotnet` only for system admins (otherwise prompt / http / shell). But **running an existing `dotnet` skill is open to all logged-in users**: any user can trial-run a server-side dotnet skill, or have a client-executed one (`ExecutionLocation=Client`) run on the local bridge — the browser cannot compile C#, so client dotnet must run over the native bridge (`AguiGroupChat.NativeBridge` `DotnetRunner`) after the triggerer approves, with no admin needed just to run (`shell`/`http` trial-runs remain admin-only). Deleting a skill referenced by digital employees releases those references (seed skills are read-only; deleting them fails).
- **Server dotnet sandbox**: `ExecutionLocation=Server` dotnet skills run inside the Hub process in a collectible `AssemblyLoadContext`, with a constrained metadata-reference allowlist, `AllowUnsafe=false`, plus timeout and output caps, to keep execution governed.
- **Native-bridge dotnet execution**: the standalone `AguiGroupChat.NativeBridge` (intranet / desktop bridge exe) ships a `DotnetRunner` and executes `kind=dotnet` tunnel tasks (Source = C#) locally — client dotnet skills run on the browser-host machine without needing pre-compiled artifacts.
- **Trial-run auto-suggests an example input**: before a “trial run” executes, the library calls `POST /ag-ui/skills/{skillId}/suggest` to auto-generate a <b>representative example input</b> derived from the skill's description / body, then runs via `POST /ag-ui/skills/{skillId}/run`—lowering the barrier to testing.
or `Client` (local / frontend / intranet tunnel)；`shell` skills are forced to require approval (`RequiresApproval=true`)，and `Client` execution (including client `dotnet`——the browser cannot compile C#，so it runs over the native bridge) always requires approval. Client-executed shell skills run on the local machine via the intranet tunnel / frontend — `ClientRunner` is auto-generated from the command body by `BuildClientRunner` (see the "Client-execution skills" section above).
...
- **Memory visualization**: user menu → “memory management” — per-group memory stats (count / latest time / expired count), browse entries by group / keyword (time / sender / level / content), per-entry <b>level / delete</b>, per-group <b>forget</b>; permission check: only own-group memory (that the user belongs to) can be viewed / governed (non-member 403). **Governable scope is role-tiered**: platform admins (Admin / SuperAdmin) may view / govern memory across every group; a group's <b>owner / admin</b> may govern <b>the whole group</b> within circles they manage (tier / delete others’ memories, group-wide forgetting, importing into that group); ordinary members may only govern <b>their own</b> posts; Operator (read-only ops) gains no memory-governance privilege from the platform role. The memory UI hints the current forget scope (whole circle / own posts only) per selected circle
