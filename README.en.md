...
- ✅ **Built-in document / presentation generation (works out of the box)**: `dotnet` skills shipped inside the assembly — no manual import — covering three Word scenarios (`docx_gongwen` official document / `docx_notice` notice / `docx_report` report) and PowerPoint (`pptx_deck`). **Pure .NET** (DocumentFormat.OpenXml), with no Node / Python / external interpreter, on the same Roslyn execution path as every other skill. `pptx_deck` organises a deck by slide type (cover / toc / section / content / two-column / table / KPI / quote / image / chart / summary / end), supports six theme presets or explicit colors and fonts, 16:9 widescreen, a page badge on every page but the cover, and optional speaker notes per slide; charts (bar / line / pie / doughnut) are **rendered to PNG and embedded as pictures** rather than emitted as ChartParts (the same call the Word skills make to avoid DrawingML chart compatibility risk). The output carries the `produce_file` marker → the gateway registers it as an attachment on the conversation, **downloadable straight from the frontend**. Seeding can be turned off with `Agents:BuiltinDocxSkills` / `Agents:BuiltinPptxSkills` (see `tools/pptx-skills/README.md`)
