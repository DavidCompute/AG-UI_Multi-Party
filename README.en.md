...
- ✅ **Enterprise compliance (data-subject rights)**: **account deletion / data erasure** (self-service “Delete account” in profile with password confirmation; admin “permanently delete” in user management; owned groups transferred or disbanded, joined groups left, message memories and personal knowledge bases erased, account row + sessions + TOTP removed, last-super-admin guarded) plus **“Export my data” portability** (`GET /ag-ui/account/export`: profile / groups / own messages / memories / knowledge bases / owned employee & skill definitions) and **audit-log search and CSV export** (`/ag-ui/admin/audit` filtered query, `/ag-ui/admin/audit.csv` RFC 4180 + UTF-8 BOM full export; Admin Console “Audit Log” tab) (see `docs/enterprise-compliance.md`)
