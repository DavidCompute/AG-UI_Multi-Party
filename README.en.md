...
**Per skill**: `pptx_deck` still falls back to Wikimedia when the library has no match, while `docx_*` / `pdf_doc` use the library only (`{"image":{"imageQuery":"…"}}` / `{"type":"image","imageQuery":"…"}`) — a miss **skips that image** and explains itself in `warnings` instead of failing the whole document (PDF embeds PNG/JPEG only and says so rather than writing a broken file).
Image order (PPT): library hit → Wikimedia fallback → generated art; `AGUI_IMAGE_SOURCE=library` makes it **fully offline**.
