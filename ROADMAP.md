...
> ✅ 标记 = 已实现；🟡 = 部分实现。当前已完成：**1.1 / 1.3 / 1.4 / 2.1 / 2.2 / 2.4 / 3.2 / 3.3 / 4.1–4.4 / 6.1 / 6.3（数据面）**；
> 3.1（自动重连）、6.3（管理 UI）为部分；**未实现**：1.2、2.3、5.1–5.4、6.2、6.4（详见各节）。
...
### 3.1 桥接健康度与自动重连（★★☆） 🟡部分（健康度探测已实现；自动重连/断线补发待续）
