...
- 「记忆与权限 → 🧠 记忆类型（拟人召回）」区行为（`agent.form.memoryType.*`）：
  ...
  - 写入侧（`AgentMessageMemory` 写队列按作者 agentId 解析 `MemoryProfile`，仅作用于该数字员工<b>本人发言</b>）：
    深记型自动升为「重要」记忆（importance=1，不随自动遗忘过期、同分检索优先）；快速遗忘型在平台开启自动遗忘时
    本人普通发言保留约全局 40%（至少 2 天）；其余类型与未配置写入完全不变。
  - 完整机制（三层记忆、五型详解、字段、调试与红线）见 `docs/memory-personality.md`。
- “技能与知识”区行为：
...
- `#imgLibModal`（图库 `modal kb-modal`）：与知识库**同构**的列表式布局——工具栏搜索（带 “×” 清除）+ 四列表格头（名称/ID · 图片 · 描述 · 操作）+ 列表 `#imgLibListWrap`（弹窗高度受 `max-height: calc(100vh - 48px)` 约束，列表区独立纵横滚动）。
  ...
  - **向量化状态徐标**（`assetBadge`）—— 用户需要知道“我写的描述到底生效了没 / 这张图能不能被匹配”：
    | 状态 | 徐标 | 含义 |
    |---|---|---|
    | `processing` | ⏳ 识别中… | 正在生成描述并向量化；**这期间这张图暂时搜不到** |
    | `ready` + 有描述 | ✅ 已向量化 | 可被语义检索命中；改描述保存后立即重新向量化并覆盖旧向量 |
    | `ready` + 无描述 | ⚠ 仅按文件名匹配 | 向量只含文件名/标签，实际找不到（未配视觉模型时上传即此态）—— 提醒用户补描述 |
    | `error` | ❌ 失败 | hover 看原因（如 embedding 不可用）；该图在修好前不会被检索命中 |
  - 图库行（收起态）在**有未就绪图片**时显示 `⚠ n 张未就绪`（`imgLib.badge.pending`），全部就绪则不显示；
...
- **`#libSetModal`（库设置，图库 / 知识库共用）**：只做一件事——**检索严格度**。
  ...
  - **🔍 试检索**（弹窗下半部）：输入一句提问 / 关键词 → 用**当前下拉里选中的档位**当场试一次，
    ...
    > 试检索只演示**语义门槛**：召回的条目都是过了当前档位的。
    > 但线上检索还有一条**不受档位管的词面兜底**（专属名词/人名/型号词面命中即召回，这是刻意的：
    > 向量表示不好的专属词不该被更严的门槛一票否决），所以实际配图时可能出来一条**低于当前档位**的命中 ——
    > 这不是“档位没生效”，两者是两道不同的闸。
  ...
  - 保存描述：`PUT /ag-ui/image-libs/{libId}/assets/{assetId}` **会在重新向量化完成后才返回**（后端 `await task`），
    所以保存按钮在此期间显示 `⏳ 向量化中…` 并禁用 —— 按钮恢复即意味着新描述已可被检索命中。
...
| 存储治理 | `GET /ag-ui/admin/storage`（附件占用 / 仍被引用数 / **无引用数（含宽限期内）** / **现就可回收数**，均需系统管理员）、`POST /ag-ui/admin/storage/reclaim`（回收无引用孤儿附件；**默认关闭**，需 `StorageGovernance:AllowReclaim=true`）|
