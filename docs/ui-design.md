...
- 工具栏：搜索 `#agentSearch`、`#agentOrgBtn`（组织架构图）、`#agentOrchBtn`（一键编排）、
  `#agentSkillLibBtn`（技能库）、导入 JSON `#agentImportBtn`、导出全部、恢复内置组织工具（管理员）、
  批量删除模式 `#agentBatchBtn`、新增 `#agentAddBtn`。
