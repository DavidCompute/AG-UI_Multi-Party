...
public sealed class GroupHub : IDisposable
{
    ...
    public async Task<bool> ResolveAgentInteractionAsync(GroupInteractionResolveRequest req, CancellationToken ct = default)
    {
        ...
        var resolved = await _agentGateway.ResolveInteractionAsync(req.InterruptId, req.MemberId!, req.Approved, req.Input, req.Payload, ct,
            approveAll: req.ApproveAll, toolResult: req.ToolResult, clientId: req.BridgeClient);
        if (resolved)
        {
