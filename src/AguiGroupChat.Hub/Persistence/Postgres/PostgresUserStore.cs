...
public sealed class PostgresUserStore : IUserStore
{
    ...
    public bool AddUser(UserAccount user)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("platformRole", UserRoleToString(user.PlatformRole));
            cmd.ExecuteNonQuery();
            return true;
        }
    ...
    public UserAccount? GetUserById(string userId)
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            SELECT user_id, username, password_hash, password_salt, nickname, avatar, created_at, updated_at,
                   personal_memory_enabled, is_admin, is_disabled, platform_role
            FROM agui_users WHERE user_id = @uid
            """;
    ...
    public UserAccount? GetUserByUsername(string username)
    {
        ...
        cmd.CommandText = """
            SELECT user_id, username, password_hash, password_salt, nickname, avatar, created_at, updated_at,
                   personal_memory_enabled, is_admin, is_disabled, platform_role
            FROM agui_users WHERE LOWER(username) = LOWER(@username)
            """;
    ...
    public IReadOnlyList<UserAccount> ListUsers()
    {
        ...
        cmd.CommandText = """
            SELECT user_id, username, password_hash, password_salt, nickname, avatar, created_at, updated_at,
                   personal_memory_enabled, is_admin, is_disabled, platform_role
            FROM agui_users ORDER BY created_at
            """;
    ...
    private static PlatformRole ReadPlatformRole(NpgsqlDataReader r)
    {
        try
        {
        ...
        return r.GetOrdinal("is_admin") is var ai && ai >= 0 && !r.IsDBNull(ai) && r.GetBoolean(ai) ? PlatformRole.Admin : PlatformRole.User;
    }
