...
/// <summary>
/// MySQL / SQLite 共用扩展区存储：把上层注册的扩展区（当前为智能体定义）序列化为 JSON 落库到 agui_sections 表。
/// 变更经 <see cref="ChangeHub"/> 标记脏位后定时合并写入（策略与 JSON 快照一致，默认 5 秒）；
/// 与上次写入内容相同时跳过，避免无关变更（如在线状态）频繁写库。
/// </summary>
public sealed class RelationalSectionStore : ISectionStore, IDisposable
{
    private readonly RelationalStore _db;
    private readonly ILogger<RelationalSectionStore> _logger;
    ...
    public RelationalSectionStore(RelationalStore db, ChangeHub changeHub, ILogger<RelationalSectionStore> logger)
    {
        _db = db;
        _logger = logger;
    ...
    private void Upsert(string name, string payload)
    {
        ...
        cmd.CommandText = _db.Dialect.Upsert(
            "agui_sections", "name, payload", "@name, @payload", "name", "payload");
        cmd.AddWithValue("name", name);
        cmd.AddWithValue("payload", payload);
        cmd.ExecuteNonQuery();
    }
