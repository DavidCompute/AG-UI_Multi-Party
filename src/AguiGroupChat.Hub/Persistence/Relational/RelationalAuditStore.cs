...
/// <summary>审计日志 MySQL / SQLite 独立表存储：表 <c>agui_audit</c>（RelationalStore.EnsureSchema 幂等建表）。</summary>
public sealed class RelationalAuditStore : IAuditStore
{
    private readonly RelationalStore _db;

    public RelationalAuditStore(RelationalStore db) => _db = db;
    ...
    public void Append(AuditEntry entry)
    {
        try
        {
            ...
            cmd.CommandText = """
                ...
                """;
            cmd.AddWithValue("id", entry.Id);
            cmd.AddWithValue("ts", entry.Timestamp);
            cmd.AddWithValue("action", entry.Action);
            cmd.AddWithValue("actorId", entry.ActorId);
            cmd.AddWithValue("actorUsername", entry.ActorUsername);
            cmd.AddWithValue("groupId", (object?)entry.GroupId ?? DBNull.Value);
            cmd.AddWithValue("targetType", (object?)entry.TargetType ?? DBNull.Value);
            cmd.AddWithValue("targetId", (object?)entry.TargetId ?? DBNull.Value);
            cmd.AddWithValue("detail", (object?)entry.Detail ?? DBNull.Value);
            cmd.AddWithValue("result", entry.Result);
            cmd.ExecuteNonQuery();
        }
        catch (Exception ex) when (_db.IsDuplicate(ex)) { /* id 冲突（幂等重放）静默忽略 */ }
    }
    ...
    public int Prune(int keepLatest)
    {
        ...
        cmd.CommandText = """
            ...
            """;
        cmd.AddWithValue("keep", Math.Max(0, keepLatest));
    ...
    private static string BuildWhere(System.Data.Common.DbCommand cmd, string? actor, string? action,
        string? targetId, long? fromMs, long? toMs)
    {
        var groups = new List<string>();
        string Like(string column)
        {
            cmd.AddWithValue($"p_{column.Replace("_", "")}", $"%{(column == "action" ? action : column == "target_id" ? targetId : actor)}%");
        ...
        if (fromMs is not null) { groups.Add("ts >= @from"); cmd.AddWithValue("from", fromMs.Value); }
        if (toMs is not null) { groups.Add("ts <= @to"); cmd.AddWithValue("to", toMs.Value); }
        return groups.Count == 0 ? "TRUE" : string.Join(" AND ", groups);
    }

    private static List<AuditEntry> ReadAll(System.Data.Common.DbCommand cmd)
    {
