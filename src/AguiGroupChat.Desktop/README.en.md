...
A pure desktop application (Windows) **(current version 1.0.142)**: it reuses all the features of the web version (group chat, agents, human-in-the-loop approval, semantic memory RAG,
...
Current version: **1.0.142** (see `<Version>` in `src/AguiGroupChat.Desktop/AguiGroupChat.Desktop.csproj`).
...
#    e.g. for 1.0.142:
powershell -ExecutionPolicy Bypass -File tools/build-msi.ps1 -Version 1.0.142
#     → artifacts/wix/AguiGroupChat-Desktop-1.0.142.msi
```
