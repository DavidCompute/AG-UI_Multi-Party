...
A pure desktop application (Windows) **(current version 1.0.143)**: it reuses all the features of the web version (group chat, agents, human-in-the-loop approval, semantic memory RAG,
...
Current version: **1.0.143** (see `<Version>` in `src/AguiGroupChat.Desktop/AguiGroupChat.Desktop.csproj`).
...
#    e.g. for 1.0.143:
powershell -ExecutionPolicy Bypass -File tools/build-msi.ps1 -Version 1.0.143
#     → artifacts/wix/AguiGroupChat-Desktop-1.0.143.msi
```
