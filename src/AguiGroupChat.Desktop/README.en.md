...
A pure desktop application (Windows) **(current version 1.0.121)**: it reuses all the features of the web version (group chat, agents, human-in-the-loop approval, semantic memory RAG,
...
Current version: **1.0.121** (see `<Version>` in `src/AguiGroupChat.Desktop/AguiGroupChat.Desktop.csproj`).
...
#    e.g. for 1.0.121:
powershell -ExecutionPolicy Bypass -File tools/build-msi.ps1 -Version 1.0.121
#     → artifacts/wix/AguiGroupChat-Desktop-1.0.121.msi
```
...
| `Agents:AllowPrivateSkillEndpoints` | `true` | Whether HTTP skills may reach <b>local / intranet / private-network</b> targets (on by default in the desktop build: a single-user local host can directly call local Ollama / intranet APIs); set `Agents:AllowPrivateSkillEndpoints=false` to re-enable strict SSRF protection so HTTP skills reject intranet targets |
| `Agents:CoordinatorPlanning` | `true` | Deterministic orchestration plan: when enabled, a router-type digital employee (with an assignment whitelist / escalation target, or <b>having mounted skills</b>) first builds an execution plan from the <b>org chart & skill config</b>, then <b>activates them in sequence</b> and aggregates the answer; <b>client-execution skills inside the plan merge into one "run all locally" card</b>, and the synthesis stage <b>recursively gathers more</b> until the info is sufficient (never asking "continue?" mid-answer); falls back to the original recursive dispatch on any failure |
...
- **Skills (two senses)**: ① attach <b>other digital employees</b> as callable sub-agents ("Skills & Knowledge → Callable sub employees") - the model auto-invokes the sub employee and cites its reply when it needs that employee's capability (including AG-UI bridged external experts); once upper/lower employees are wired this way, an upper employee can trigger a lower employee's capability on demand. ② configure reusable shell / http / prompt skills in the Skill Library and have digital employees mount them via `skillDefIds`.
  ...
  shell skills stay **always requiring approval** because they run arbitrary local commands; agents can also build skills themselves via the `create_skill` tool (requires approval). The Skill Library also supports **generate a skill from plain text** (describe the request; the LLM fills in name/command/description/execution location, ready to tweak and save);
- **Client-execution skills (`ExecutionLocation=Client`)**: a skill can be marked to <b>run on the local machine</b> - in the desktop edition the desktop shell (the local process) runs shell in a sandbox or the WebView runs http, and the result is fed back to the model (desktop needs <b>no extra native-bridge/token config</b>; works out of the box). Multiple client skills selected by an orchestration plan merge into one "run all locally" card after a single confirmation.
