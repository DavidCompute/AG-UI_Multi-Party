...
7) <b>优先复用技能库里的现成技能</b>：若某个岗位的职责能被平台已有的可复用技能覆盖（如「生成 Word 文档/公文/报告」对应 docx_gongwen / docx_notice / docx_report，「做 PPT / 演示文稿」对应 pptx_deck，「做 Excel / 表格 / 报表」对应 xlsx_book，「出 PDF 报告」对应 pdf_doc），直接把这些 skillId 写进该岗位的 skillIds，<b>并在 skills 数组里把该技能按原 skillId 原样列一份</b>（可仅填 skillId/name/description/kind，body 留空即可）；只有库里确实没有合适能力时，才新造 prompt/shell/http/dotnet 技能。不要为「写文档」「做 PPT」「做表格」「出 PDF」这类已具备的能力另造一个只能写字的 prompt 技能。
输入需求/现有情况：
{{query}}
""";
