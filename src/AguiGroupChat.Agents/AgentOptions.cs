...
public sealed class AgentOptions
{
    ...
    ///
    /// <para>
    /// 为什么要单独给一个更长的值：这些技能除了排版还有别的重活 —— 比如 PPT 的 <c>imageQuery</c>
    /// 要逐张做图库语义检索（本机回环，快但非免费；历史上“回落 Wikimedia 检索 + 下载照片”
    /// 每张几张百 KB，实测 3 张就撞上 10 秒上限）。而超时的结果是<b>整份稿子都没有</b>
    ///（比降级成题图差得多）。注：联网取图自 1.0.164 起默认关闭，但预算仍按“最重的那条路”留。
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// 是否播种<b>内置 PPT 生成</b>技能（<c>pptx_deck</c>：封面/目录/章节/内容/表格/图表等页型）。
    /// <c>null</c>（默认）= 开启。设 <c>false</c> 则不播种（适合不需要演示文稿能力的部署）。
