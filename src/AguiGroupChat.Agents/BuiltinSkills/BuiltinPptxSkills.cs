...
public static class BuiltinPptxSkills
{
    ...
    [
        (
            ...
            "【真照片配图：imageQuery（来源 = 团队图库，不联网）】用户说要“照片/实景图/每页配图”，或希望画面更像真实场景时，" +
            "在需要图的位置写 \"imageQuery\": \"关键词\"，工具会用平台**团队图库**（用户自己上传的图片）做语义检索，" +
            "命中即嵌入那张图、且**不需要任何署名**；" +
            ...
            "若关键词没命中，工具会**再拿本页文字（含人名/奖项名）切短后重查图库**，" +
            "所以页面上写着谁的名字，就优先配上他本人的照片。" +
            "**本平台当前只从团队图库配图、不联网检索**（联网取图已下线）：命中的是贵方自有素材，" +
            "**不需要署名**，所以正常出稿**不会有「图片来源」页**。图库里没有匹配的图（或该岗位没绑图库）时" +
            "会降级为自动题图并在 warnings 说明，**不是失败**。请按图库可能已有的素材来写关键词，" +
            "**不要向用户暗示会拿到网图/实景照片**，也不要为此反复重试。" +
            ...
            "imageSource(可选，配图来源：默认 library = 只查团队图库、不联网；auto = 先图库后网络；" +
            "network = 仅网络。平台当前默认只查图库、联网取图已下线，**非必要不要传 auto / network**)、" +
            "imageSearchApi(可选，仅在显式开启联网取图时才有意义：覆盖照片检索端点；" +
            "也可用环境变量 AGUI_PHOTO_API 指向镜像/代理)、" +
            ...
            "【规模与分批（避坑）】单次生成建议 **≤ 25 页**；**配图（imageQuery）建议 ≤ 6 张**" +
            "——配图每张都要做一次图库语义检索（本机回环调用，快但非免费；旧版联网取图实测 4 张就吃掉 45 秒），" +
            "点多了同样容易撞上执行时间预算（超时=整份稿子都没了）。" +
