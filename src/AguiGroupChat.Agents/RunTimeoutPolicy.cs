...
/// 拿不到触发上下文的组件（技能宿主 <c>SkillRunner</c>）。
///
/// <para>
        ...
        ExecutionOptions execution, AgentOptions options)
    {
        ...
        if (!opt.Enabled)
            return new RunTimeoutBudget(RunComplexity.Simple, 0, 1, baseMinutes, baseMinutes,
                baseSkillMs, BaseClientSkillTimeoutSec, []);
        ...
        return new RunTimeoutBudget(tier, score, multiplier, baseMinutes, ceiling, skillMs, clientSec, reasons);
    }
