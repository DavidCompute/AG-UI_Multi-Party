...
/* ============ 内置管理入口：托管团队（整支覆盖 / 删除 / 只留最新）—自包含，不依赖其它导入 ============ */
(function () {
  let omgOverlay = null;
  let omgText = null; // 共享的 plan(JSON) 输入
  const btnId = "omgManageBtn";
  const overlayId = "omgOverlay";

  function hasToken() { return !!(typeof state !== "undefined" && state && state.token); }

  async function authFetch(url, method, body) {
    const headers = { Authorization: "Bearer " + state.token };
    if (body !== undefined) headers["Content-Type"] = "application/json";
    const res = await fetch(url, { method, headers, body: typeof body === "string" ? body : (body === undefined ? undefined : JSON.stringify(body)) });
    let data = null; try { data = await res.json(); } catch {}
    return { ok: res.ok, status: res.status, data };
  }

  function notify(msg, isErr) {
    try { if (typeof toast === "function") { toast(msg === undefined ? msg : (isErr ? "⚠️ " + msg : "✅ " + msg)); return; } } catch (e) {}
    if (isErr) alert(msg);
  }

  function esc(s) { return (s || "").replace(/[&<>”]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "”": "&quot;" }[c])); }

  async function refresh(tbody, emptyMsg) {
    const r = await authFetch("/ag-ui/agents/org-teams", "GET");
    if (!r.ok) { tbody.innerHTML = `<tr><td colspan="5" class="muted">${esc(r.data?.message || "加载失败（仅系统管理员可见）")}</td></tr>`; return; }
    const rows = (r.data || []);
    if (!rows.length) { tbody.innerHTML = `<tr><td colspan="5" class="muted">${esc(emptyMsg)}</td></tr>`; return; }
    tbody.innerHTML = rows.map(row => {
      const ud = row.updatedAtMs ? new Date(row.updatedAtMs).toLocaleString() : "";
      return `<tr>
        <td>${esc(row.key)}</td><td>${esc(row.title || "")}</td>
        <td class="muted" style="font-size:12px">${esc((row.agents || []).join(", ") || "—")}</td>
        <td class="muted" style="font-size:12px">${esc((row.skills || []).join(", ") || "—")} / ${esc(ud)}</td>
        <td style="white-space:nowrap">
          <button data-omg="apply" data-key="${esc(row.key)}" class="chip-btn">覆盖</button>
          <button data-omg="del" data-key="${esc(row.key)}" class="chip-btn" style="color:#e5484d">删除</button>
        </td></tr>`;
    }).join("");
    tbody.querySelectorAll("button[data-omg]").forEach(b => b.addEventListener("click", async () => {
      const key = b.dataset.key;
      if (b.dataset.omg === "del") {
        if (!confirm(`删除托管团队「${key}」？会退役其当前对象并从托管列表移除。`)) return;
        const dr = await authFetch("/ag-ui/agents/org-teams/" + encodeURIComponent(key), "DELETE");
        notify(dr.ok ? "已删除：" + key : (dr.data?.message || "删除失败"), !dr.ok);
        await refresh(tbody, emptyMsg);
        return;
      }
      const plan = (omgText ? omgText.value : "").trim();
      if (!plan) { notify("请先在下方粘贴最终方案 JSON", true); return; }
      let parsed;
      try { parsed = JSON.parse(plan); } catch (_) { notify("方案 JSON 无法解析", true); return; }
      const ar = await authFetch("/ag-ui/agents/org-teams/" + encodeURIComponent(key) + "/apply", "POST", JSON.stringify(parsed));
      notify(ar.ok ? (ar.data?.message || ("已整支覆盖：" + key)) : (ar.data?.message || "覆盖失败"), !ar.ok);
      if (ar.ok) await refresh(tbody, emptyMsg);
    }));
  }
  ...
  function buildOverlay() {
    ...
    ov.id = overlayId;
    ov.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;z-index:9999;display:none";
    ov.innerHTML = `<div class="modal" style="max-width:760px;width:92vw;max-height:86vh;overflow:auto;display:flex;flex-direction:column">
      <div class="modal-actions" style="margin-bottom:10px">
        <span style="flex:1;font-weight:600" data-i18n="org.orchTitle">托管团队 · 整支覆盖仅系统管理员</span>
        <button id="omgClose" class="btn-ghost">关闭</button>
      </div>
      <textarea id="omgPlan" rows="6" class="modal-input" style="width:100%;font-family:monospace;font-size:12px;white-space:pre-wrap" placeholder="在此粘贴要应用的最终方案 JSON（结构同【一键编排】最终稿）…"></textarea>
      <div style="margin-top:8px;font-size:12px" class="muted">选择一个 key，点其“覆盖”即把上方方案整支应用到该 key（先删上一版、再落最新一版，库里只留最新）；连“删除”可清除整支与托管记录。</div>
      <div style="overflow:auto;margin-top:8px"><table class="table" style="width:100%;border-collapse:collapse">
        <thead><tr><th>Key</th><th>标题</th><th>数字员工</th><th>技能 / 更新</th><th>操作</th></tr></thead>
        <tbody id="omgRows"></tbody></table>
      </div>
    </div>`;
  ...
  function openUI() {
    if (!hasToken()) { notify("请先登录", true); return; }
    if (!omgOverlay) {
      omgOverlay = buildOverlay();
      omgOverlay.querySelector("#omgClose").addEventListener("click", () => { omgOverlay.style.display = "none"; });
      omgOverlay.addEventListener("click", (e) => { if (e.target === omgOverlay) omgOverlay.style.display = "none"; });
      omgText = omgOverlay.querySelector("#omgPlan");
    }
    omgOverlay.style.display = "flex";
    refresh(omgOverlay.querySelector("#omgRows"), "还没有托管团队。先用【一键编排】落一支后，这里即可查看并整支覆盖/删除。");
  }

  function install() {
    const tb = document.getElementById("agentSkillLibBtn") && document.getElementById("agentSkillLibBtn").parentElement;
    if (!tb || document.getElementById(btnId)) return;
    const b = document.createElement("button");
    b.id = btnId;
    b.className = "chip-btn";
    b.textContent = "🧩 托管团队";
    b.style.cssText += ";color:#8a5cf6";
    b.addEventListener("click", openUI);
    document.getElementById("agentSkillLibBtn").insertAdjacentElement("afterend", b);
  }
