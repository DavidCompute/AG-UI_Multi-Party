...
- **Memory personality types (type-driven recall)**: every digital employee can configure its own memory characteristics — five human-like presets (**Broad / Deep / Slow-to-learn / Stored-but-cue-dependent / Fast-forgetting**) plus a speaking style, a persona tone and TopK / threshold tuning. Recalling shared group / personal memory is then genuinely executed per type (hit count, similarity threshold, recall-cue branch, recency window); a Deep employee's own posts are auto-marked “Important”, and a Fast-forgetting employee's own ordinary posts are kept shorter when auto-forget is on — “the same circle, yet each digital employee remembers it in its own way” (see `docs/memory-personality.md`). Orgs built in one click follow the same idea: the generator auto-assigns each role a fitting personality preset, so every newly orchestrated employee already “remembers in its own way” the day it joins.
...
- **Memory personas auto-assigned (implemented)**: one sentence no longer builds a list of job titles — it builds a team whose members each remember in their own style: lead supervisors get a Deep profile, high-volume frontline / customer-service roles go Broad, account-success / after-sales / consultants are cue-dependent, quick-lookup duty shifts are Fast-forgetting, and repetitive routine roles are Slow-to-learn. Each employee's memory type is tagged in the preview before you commit, and it takes effect the moment the org is applied.
- **Connections auto-paired (implemented)**: hand the system one requirement and it guarantees both directions of the org's division of labor — every manager can assign tasks downward (task assignment) and every subordinate can escalate problems upward (escalation), so a generated team never collapses into a one-way chain where problems only fly up and nobody hands down the work.
...
- One-click orchestration generates and manages “roles × per-role skills × up/down connections” together (assign-down / escalate-up / handoff), yielding a digital team that runs like a real company without separately maintaining employees and skills and glueing them. Every generated role also ships with a matched memory personality, and its assign-down / escalate-up connections are auto-paired — so from the moment it lands the team can take work, report upward, and remember.
...
- <b>“Build the structure, get the capability” is not a slogan — the deliverable really lands in your hands.</b> The platform ships four built-in producers: Word (official document / notice / report), Excel (`xlsx_book`: multiple worksheets, formulas first, number formats, totals rows, frozen panes and autofilter), PowerPoint (`pptx_deck`: twelve slide types, six themes, 16:9, speaker notes, and **every slide can carry a real, content-matched photograph**: an `imageQuery` semantically matches the <u>team image library</u> — your own pictures, no attribution needed, and no internet required at all (web image search was retired in 1.0.164, so illustration is library-only and no “Image credits” page shows up); Word and PDF take `imageQuery` from the library too) and PDF (`pdf_doc`: several document layouts and cover styles, a content-block system, Markdown re-typesetting), with **no Node, Python or external interpreter**, and the output is attached back to the conversation — **downloadable and also readable right in the page**: a **👁 View online** button expands a reader in place, with no download and no Office install (the server converts to PDF and renders it inline; permissions are identical to downloading, so non-members can never read it). Orchestration <b>writes “deck → `pptx_deck`, Excel → `xlsx_book`, PDF → `pdf_doc`, Word → `docx_report`” into the delivery role automatically</b> and refuses to let a text-only prompt skill masquerade as a delivery capability; if nobody in the plan can produce the file format you asked for, the preview warns you up front — so you never wait around only to receive plain text.
...
> **Already shipped**: one-click organization orchestration (with SSE streaming generation) & one-click support-circle creation, support-circle conversation isolation, support-circle typing & agent context, auto-rename on orchestration id collisions, org-chart double-click edit & line de-overlap, platform-role RBAC, native-skill tunnel execution, **auto-assigned memory personas in organization orchestration**, and **auto-paired assignment / escalation connections** are all live; the following are directions to deepen next.
...
**· Innovation inventory vs. comparable products (tiered)**

Measured against multi-agent frameworks (AutoGen / CrewAI / LangGraph), AI-bot / low-code platforms (Coze / Dify / Copilot Studio), enterprise RAG / knowledge platforms, and traditional customer-service AI: most peers deliver one or two of these items, but very few combine them into a **single runnable, governable, extensible product** the way KnowGath does.

**A. Structural innovations (lead with these externally — 8 total)**
1. **A group chat that is a command room + an org runtime**: other multi-agent products are code orchestration or an unordered list of bots; here each group is a "digital-employee department" — **assignment / escalation / handoff are not just pictures; the runtime truly routes work along the org relations**.
2. **One sentence builds a runnable organization**: requirement → roles + per-role skills + up/down connections → SSE-streamed preview → server-side smoke test & auto-repair → atomic apply → auto-dedup on name collisions, **connections auto-paired (assignment + escalation)**, **memory personas auto-assigned** → optionally landed directly as a live support circle.
3. **Human-like memory personalities + org auto-assignment**: the five presets (Broad / Deep / Slow-to-learn / Stored-but-cue-dependent / Fast-forgetting) genuinely change retrieval hit counts, similarity thresholds, recall-cue branches and recency windows, with only conservative write-side tuning and a hard rule of **never fabricating memories**; org building auto-assigns a fitting profile per role, so an employee "remembers in its own way" from day one.
4. **An organization that builds itself in conversation**: the org architect is itself a group member digital employee — @ it in the group and it creates more digital employees into the catalog through gated tools (admin-gated, same team-key overwrite keeps only the latest); the org evolves through dialogue.
5. **Support circle: multiple AI employees directly facing customers**: publicly enterable, one-to-one isolated customer sessions, internal staff chat that never disturbs customers, **customers may approve the local skills their request triggered** — and it can be spun up from one sentence. It pushes an agent-group product into a customer-facing shape.
6. **Approved "hands-on" action on the local machine from chat**: client skills execute real shell and C# over the native bridge / tunnel — C# is dynamically compiled with dynamic NuGet restore and self-repair on compile failure. AI does more than talk: it can act on your computer, controlled and audited.
7. **An open protocol ecosystem instead of model lock-in**: AG-UI Group-Chat Extension Protocol v1.0 + MSAGENT alignment + external-expert bridging + Hub cascading (cross-organization / cross-network collaboration and approval); external AG-UI employees can join a group as "staff".
8. **Governance built into the chat itself**: approval cards inline in the conversation, differentiated approval policies, four-tier platform RBAC, operational audit (search / CSV / dedicated table), account deletion & erasure / orphan governance, TOTP and instant session revocation — a rare "enterprise-ready compliance kit" for an AI collaboration product.

**B. Combined, experience-level highlights (8)**
- Full semantic-memory productization: tiering / auto-forgetting / timeline playback / visual governance / cross-instance incremental sync (memory-as-a-data-pack) / Graph RAG / hybrid BM25 + vector / auto-sinking important conclusions into the knowledge base;
- Role-tiered memory-management scope (platform admin / group owner / ordinary member each govern a different range);
- Orchestration plans that recursively gather until the information is sufficient, with multiple client skills merged into a single "run all locally" card;
- Role handoff, scheduled/cron tasks, global intelligent search, topic isolation and cross-topic linking;
- AI twin (offline standby, public-group follow);
- Visual org chart: double-click a node to edit, lane-offset lines, and **"optimize dispatch" auto-writing next-layer routing guidance into Instructions**;
- Natural-language skill generation + smoke test + auto-repair + suggested trial-run inputs;
- One source for many clients + data sovereignty: Web (Docker) / Windows desktop / cross-platform desktop share the same Hub; SQLite + local embeddings work offline; five storage backends are switchable; desktop and cloud memories can be migrated across instances.

**Messaging guidance**: this inventory is an internal review dated 2026-09 based on actual shipped capabilities; individual items already exist in or can be copied by peers, so marketing copy should say "**exclusive combination / rarely seen together**" rather than "first / unique". The external story can lead with **six differentiators**: ① a one-sentence runnable digital org, ② human-like memory with auto-assignment, ③ support circles that face customers directly, ④ approved on-machine actions from chat, ⑤ governance & compliance built in, ⑥ an open protocol ecosystem.

**· Comparison by competitor class**

| Dimension | Multi-agent frameworks (AutoGen / CrewAI) | AI-bot / low-code platforms (Coze / Dify) | Enterprise RAG / knowledge platforms | **KnowGath** |
|---|---|---|---|---|
| Interface | Code / SDK; no end-user UX | Web builder + chat bot | Document-backend Q&A | **Group-chat IM (human & machine on one thread), usable instantly** |
| Org form | Task graph in code | Single bot / simple multi-step flow | None | **Digital-employee org (roles + skills + assignment / escalation / handoff), truly scheduled at runtime** |
| Creation | Hand-written agent graph | Drag-and-drop / manual bot setup | Manual document import | **One sentence → a runnable org (SSE preview, self-test & repair, atomic apply, auto-paired connections, auto-assigned memory personas)** |
| Memory | Session / vector store (developer-managed) | Limited platform memory | Mainly document retrieval | **Shared group semantic memory (tiering / forgetting / timeline playback / visual governance / cross-instance migration / graph) + five human-like memory profiles** |
| What it can do | Call APIs / code | Platform plugins | Answer from documents | **Beyond chat & APIs: run shell and C# on the triggerer's machine with approval (dynamic compile, NuGet restore, self-repair), via tunnel / native bridge** |
| Open ecosystem | Depends on each framework's ecosystem | Platform lock-in | Private | **AG-UI open protocol + external-expert bridging + Hub cascading (cross-org collaboration and approval)** |
| Governance & compliance | Weak | Medium | Medium | **Inline approval cards, differentiated approvals, layered RBAC, audit (search + CSV + dedicated table), erasure / orphan governance, TOTP / session revocation** |

**· Why an “organization” rather than a harness (product philosophy)**

Mainstream agent harnesses (tool routers / function calling / the LangChain-LangGraph family) are shaped like **one brain + many hands**: every tool, retrieval call and even sub-agent is reduced to “invoke once, get one result back”, and all world knowledge ultimately funnels into **a single context window**. Their strengths are simplicity, control and deep single-task focus — but once the world turns cross-domain and answers need multiple viewpoints, the single brain hits three ceilings at once: the context window overflows, tools/knowledge pile up without role-based governance, and long-term memory cannot diverge by domain.

KnowGath raises the same problem by one level: **many brains with a division of labor + organizational memory**. Each digital employee is an “organization member” with a persistent identity, a responsibility boundary, its own knowledge-base and skill mounts, and its own memory personality. It **assigns** work that is not its own down to subordinates, **escalates** what it cannot handle up to a superior, and **hands off** whole rounds to the right counterpart; when one member does not know something, it finds the member who does — instead of stuffing every tool into its own prompt.

This philosophy carries three hard constraints that must be honored:
1. **The organization is mechanism, not skin**: assignment / escalation / handoff are not just lines on an org chart — they are the real basis for runtime routing, approval boundaries and accountability; human-like memory is not wording but a “memory personality” that genuinely changes retrieval and writes.
2. **World cognition comes from role-based access, not magic**: the organization does not become “wiser about the world” by itself — it mounts the access points to the world (knowledge bases / skill library / local-machine execution / external experts) **per responsibility**, so each member only carries its own domain, and multi-role synthesis merges distributed cognition into a conclusion. The cognition boundary therefore stays extensible, handoff-able and auditable.
3. **No dogma — single brain and organization coexist**: a single hard task lets one employee go deep (personal-assistant mode); cross-domain / multi-view tasks take the org path; orchestration plans recursively gather until the information is sufficient. The choice belongs to the system, not to a camp.

The product value that follows: context cost is amortized across members; a new capability means **adding a role rather than rewriting the harness**; “who did it, using whose memory and skills” is naturally auditable at every step; and the organization can build and evolve itself in conversation (the org architect creates new employees straight into the catalog). In one sentence: **turn “tools one person can call” into a world that a group of people — each with their own specialty, able to hand off to one another, and able to remember — can touch; and make every such touch divisible, governable and recorded.**
