...
- <b>“Build the structure, get the capability” is not a slogan — the deliverable really lands in your hands.</b> The platform ships built-in Word (official document / notice / report) and PowerPoint (`pptx_deck`: cover / toc / section / two-column / table / KPI / chart / summary and more across twelve slide types, six themes, 16:9, speaker notes) producers, <b>written in pure .NET with no Node, Python or external interpreter</b>, and the output is attached back to the conversation for download from the web UI. Orchestration <b>writes “deck → `pptx_deck`, Word → `docx_report`” into the delivery role automatically</b> and refuses to let a text-only prompt skill masquerade as a delivery capability; if nobody in the plan can produce the file format you asked for, the preview warns you up front — so you never wait around only to receive plain text.
