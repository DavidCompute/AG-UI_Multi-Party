...
// ============================================================================
// docx_notice —— 通知 / 公告 / 说明（简洁单页优先）
//
// 【何时使用】生成对外通知、公告、事项说明、操作指引等短文档时使用。标题醒目、正文不缩进、层次精简、优先单页呈现。
// 【可用内容块】heading / paragraph / bullets / numbered / quote / table / image(path|imageQuery) / chart / toc / pageBreak
//
...
{
    // ===== 场景排版参数（由生成器注入）=====
    private const string SceneName = "notice";
    private const string FontTitle = "微软雅黑";
    private const string FontHeading = "微软雅黑";
    private const string FontBody = "宋体";
    private const int SizeTitle = 40;
    ...
    private const int LineBody = 360;
    private const bool TitleBold = true;
    private const bool HeadingBold = true;
    private const bool BodyIndent = false;
    private const bool UsePageNumbers = false;
