...
| 顺序 | 来源 | 命中时 | 拿不到时 |
|---|---|---|---|
| ① | **团队图库**（用户上传的图，平台按语义检索） | 用**本地文件**直接嵌入，**不需要署名** | 走 ② |
| ② | 程序化题图 | 按主题配色生成抽象图，并在 `warnings` 说明 | — |

> **联网取图（Wikimedia Commons）自 1.0.164 起暂时下线**：上面原本的“② 网络检索”不再走，
> 配图只从图库来，因而不依赖外网、也**不会再出现「图片来源」署名页**。
> 代码与端点覆盖（`imageSearchApi` / `AGUI_PHOTO_API`）都还在，要恢复**必须显式**指定
> `imageSource=auto`（入参）或 `AGUI_IMAGE_SOURCE=auto`（环境变量）——旧版默认就是 auto。
...
`library`（**默认**，仅图库、彻底不出网）/ `auto`（先图库后网络）/ `network`（仅网络）。
**只有显式写 `auto` / `network` 才会出网**（拼写错误的值也当 `library`）。
...
2. **本页文字兜底**：模型关键词完全没命中时，用本页文字（切短）再查一次图库
   —— 页面上写着“高效习惯优秀进步奖 · 刘佳俊”，这个名字就是最好的检索词。
...
> **“只在取图前查一次预算”是不够的**（实测踩到）：单次取图内部可能包含库检索 10 秒 + 网络检索 8 秒 +
> 逐个候选下载 12 秒 × N，合计能**越过预算好几倍**。实测（容器里跑真流程）：**4 张配图 45 秒、
> 8 张正好撞在 60 秒硬预算上** → 整份稿子都没了，用户看到的是
> “生成超时了，我把页数收敛后重出一版”（白跑一轮）。
>
> 现在**每次取图网络调用的超时都夹到剩余预算内**（`ClampToBudget`，剩余不到 3 秒就不开工），
> 取图阶段最多花掉预算本身。回归：`PhotoBudget_CapsEachNetworkCall_SoTheDeckStillCompletes`
> （桩每条请求故意挂 30 秒、预算压到 2 秒，断言整个技能几秒内收手并如实报预算用尽）。
> 取图预算可用 `AGUI_PHOTO_BUDGET_SEC` 覆盖（部署级调参）。
...
**页数不是问题，配图才是**（实测，容器里跑真流程）：

| 稿子 | 耗时 |
|---|---|
| 纯文本 41 页（含长备注） | **5.5 秒** |
| 12 页带 4 张联网配图 | **45 秒** |
| 12 页带 8 张联网配图 | **60 秒**（撞预算） |

所以技能描述里直接告诉了模型规模建议：单次 **≤ 25 页**、联网配图 **≤ 6 张**，
更大的稿子分批：先生成主体，再用 `action:"edit"` + `op:"append"` 追加（同一个渲染器，观感一致）。
超时时返回的文案也不再只是一句“已中止”，而是直接给出“不要原样重试、减少配图/页数或分批 append”。
...
### 部署注意：联网取图默认已关（本来还有“Wikimedia 不可达”问题）

自 1.0.164 起联网取图已**暂时下线**，默认只查团队图库，所以内网部署不必再担心下面这个问题。
若日后要恢复：实测本环境的容器能访问 `example.com`（200）与 `api.nuget.org`（302），但
