/**
 * 把 xlsx 技能同步到平台内置副本（嵌入资源）。
 * ---------------------------------------------------------------------------
 * 与 tools/pptx-skills/sync-builtin.mjs、tools/docx-skills/sync-builtin.mjs 同一职责：
 * 内置技能是仓库里技能正文的一份拷贝，作为嵌入资源随程序集分发。改完 xlsx_book.cs
 * 必须同步，否则「界面上的技能」与「新部署自带的技能」会不一致。
 *
 * 用法：node tools/xlsx-skills/sync-builtin.mjs
 *
...
/** 仓库里的技能正文 → 内置同名技能 id */
const FILES = [["xlsx_book.cs", "xlsx_book"]];
